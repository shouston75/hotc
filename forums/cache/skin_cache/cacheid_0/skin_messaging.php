...
//<![CDATA[
	ipb.templates['invite_more'] = new Template(\"<form method='post' action='" . $this->registry->getClass('output')->formatUrl( $this->registry->getClass('output')->buildUrl( "app=members&amp;module=messaging&amp;section=view&amp;do=addParticipants", 'public','' ), "", "" ) . "'>\" +
									...
									\"<fieldset class='row1'><h3>{$this->lang->words['invite_more_part']}</h3>\" + 
									\"<ul><li class='field'><label for='invite_more_autocomplete'>{$this->lang->words['enter_member_names']}</label>\" + 
									\"<input type='text' class='input_text' name='inviteNames' id='invite_more_autocomplete' size='50' />\" + 
									\"<span class='desc'>#{name_text}{$this->lang->words['separated_with_commas']}</span>\" + 
									\"</li></ul></fieldset><fieldset class='submit'><input type='submit' class='input_submit' value='{$this->lang->words['part_add']}' /> \" + 
									\"{$this->lang->words['or']} <a href='#' class='cancel' title='{$this->lang->words['cancel']}'>{$this->lang->words['cancel']}</a></fieldset></form>\");
//]]>
</script>
...
<div id='messenger_utilities' class='left'>
	...
	" . ((is_array( $topicParticipants ) and count( $topicParticipants )) ? ("<div class='general_box alt' id='participants'>
			<h3 class='bar'>{$this->lang->words['participants']}</h3>
				...
				".$this->__f__0c1d8ceec779bcb21ad3fab5c9014e2b($html,$jumpmenu,$dirData,$totalData,$topicParticipants,$inlineError)."					
			</ul>
	...
	<div id='folder_list' class='general_box alt'>
		...
		<ol id='folders'>
			" . ((count($dirData)) ? ("
				".$this->__f__a6016d61e60a1faf4fe8764bd2f4475d($html,$jumpmenu,$dirData,$totalData,$topicParticipants,$inlineError)."			") : ("")) . "
		</ol>
		<ul class='post_controls'>
...
function __f__0c1d8ceec779bcb21ad3fab5c9014e2b($html, $jumpmenu, $dirData, $totalData=array(), $topicParticipants=array(), $inlineError='')
{
	$_ips___x_retval = '';
...
function __f__a6016d61e60a1faf4fe8764bd2f4475d($html, $jumpmenu, $dirData, $totalData=array(), $topicParticipants=array(), $inlineError='')
{
	$_ips___x_retval = '';
...
$IPBHTML .= "<form id='pm_to_member_{$toMemberData['member_id']}'>
	...
	<fieldset class='submit'>
		<input type='submit' class='input_submit' name='send_msg' value='{$this->lang->words['quick_send_button']}' /> {$this->lang->words['or']} <a href='#' class='cancel' title='{$this->lang->words['cancel']}' id='pm_cancel_{$toMemberData['member_id']}'>{$this->lang->words['cancel']}</a>
	</fieldset>
</form>";
return $IPBHTML;
}
		...
		" . ((intval($this->memberData['g_max_mass_pm'])) ? ("
			...
			<li class='field'>
				...
				<select name='sendType' id='send_type' tabindex='0'>
					<option value='invite'>{$this->lang->words['send__invite']}</option>
					<option value='copy'>{$this->lang->words['send__copy']}</option>
				</select>
				<span class='desc'>
...
<fieldset class='submit'>
	...
	") : ("
		<input class='input_submit' type=\"submit\" value=\"Save Message\" tabindex=\"0\" accesskey=\"s\" />
...
$IPBHTML = "";
$IPBHTML .= "" . $this->registry->getClass('output')->getTemplate('global')->include_lightbox() . "
...
<div class='generic_bar'></div>".$this->__f__d10ae581869dcf508dc4253eff5ec6f6($topic,$replies,$members,$jump)."<br />
...
function __f__d10ae581869dcf508dc4253eff5ec6f6($topic, $replies, $members, $jump="")
{
	$_ips___x_retval = '';
		...
		<div class='post_wrap'>
			...
			")) . "
					...
					" . (($msg['_ip_address'] != '') ? ("<span class='ip'>({$this->lang->words['ip']}:
						...
						") : ("
							<a href=\"" . $this->registry->getClass('output')->formatUrl( $this->registry->getClass('output')->buildUrl( "app=core&amp;module=usercp&amp;tab=members&amp;area=mod_ipaddress&amp;do=custom&amp;ip={$msg['ip_address']}", 'public','' ), "", "" ) . "\" title='{$this->lang->words['find_info_about_ip']}'>{$msg['ip_address']}</a>
						")) . "
						)</span>") : ("")) . "
				</h3>
...
<div id='message_list'>
	...
	<input type=\"hidden\" name=\"auth_key\" value=\"{$this->member->form_hash}\" />
		<table class='ipb_table' id='message_table'>
			...
			" . ((count( $messages )) ? ("
								".$this->__f__5beba97ae4dd450c55301e8016a83f74($messages,$dirname,$pages,$currentFolderID,$jumpFolderHTML)."			") : ("
				<tr>
...
function __f__08a739f86196c6ea626b5b5979671a93($messages, $dirname, $pages, $currentFolderID, $jumpFolderHTML,$id='',$msg='')
{
	$_ips___x_retval = '';
...
function __f__5beba97ae4dd450c55301e8016a83f74($messages, $dirname, $pages, $currentFolderID, $jumpFolderHTML)
{
	$_ips___x_retval = '';
						...
						<td>
							...
							<span class='m_title'>
								...
								" . ((is_array( $msg['pages'] ) AND count( $msg['pages'] )) ? ("
									...
									".$this->__f__08a739f86196c6ea626b5b5979671a93($messages,$dirname,$pages,$currentFolderID,$jumpFolderHTML,$id,$msg)."									</ul>
								") : ("")) . "
...
<div id='message_list'>
		<table class='ipb_table' id='message_table'>
			...
			" . ((count( $messages )) ? ("
								".$this->__f__54ee0b42c0e892ccf006ca6468e41e35($messages,$pages,$error)."			") : ("
				<tr>
...
function __f__54ee0b42c0e892ccf006ca6468e41e35($messages, $pages, $error)
{
	$_ips___x_retval = '';
	foreach( $messages as $id => $msg )
	{
