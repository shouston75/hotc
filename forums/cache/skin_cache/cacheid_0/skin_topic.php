...
<div class='general_box alt poll' id='poll_{$poll['pid']}'>
	<form action=\"" . $this->registry->getClass('output')->formatUrl( $this->registry->getClass('output')->buildUrl( "app=forums&amp;module=extras&amp;section=vote&amp;t={$topicData['tid']}&amp;st={$this->request['st']}&amp;do=add", 'public','' ), "", "" ) . "\" name='pollForm' method=\"post\">
		...
		") : ("")) . "
		".$this->__f__b3021b2c3e82e11ed5a1704c9ec1895f($poll,$topicData,$forumData,$pollData,$showResults)."		<fieldset class='submit'>
...
function __f__145e34b985e8d0c699915671a5301047($poll, $topicData, $forumData, $pollData, $showResults,$questionID='',$questionData='',$choiceID='',$choiceData='')
{
	$_ips___x_retval = '';
...
function __f__e667c6915aef64a843222409fadf8ba1($poll, $topicData, $forumData, $pollData, $showResults,$questionID='',$questionData='')
{
	$_ips___x_retval = '';
							...
							" . (($showResults) ? ("<li>
									...
									" . (($poll['poll_view_voters'] AND is_array( $choiceData['voters'] ) AND $this->settings['poll_allow_public'] AND $choiceData['votes']) ? ("
										<script type='text/javascript'>
											...
											".$this->__f__145e34b985e8d0c699915671a5301047($poll,$topicData,$forumData,$pollData,$showResults,$questionID,$questionData,$choiceID,$choiceData)."											ipb.topic.poll[ $questionID ][ $choiceID ] = { name: \"{$choiceData['choice']}\", users: users};
										</script>
									") : ("")) . "
...
function __f__b3021b2c3e82e11ed5a1704c9ec1895f($poll, $topicData, $forumData, $pollData, $showResults)
{
	$_ips___x_retval = '';
	foreach( $pollData as $questionID => $questionData )
	{
		...
		$_ips___x_retval .= "
			<div class='poll_question" . (($showResults) ? (" voted") : ("")) . "'>
				...
				") : ("
					<ol>
						".$this->__f__e667c6915aef64a843222409fadf8ba1($poll,$topicData,$forumData,$pollData,$showResults,$questionID,$questionData)."					</ol>
				")) . "
			</div>
			<script type='text/javascript'>
...
$IPBHTML .= "<div id='attach_wrap' class='rounded clearfix'>
	<h4>$title</h4>
	<ul>
		".$this->__f__1b109395d20779ff61f638c5b2017261($title,$data,$type)."	</ul>
</div>";
return $IPBHTML;
}


function __f__1b109395d20779ff61f638c5b2017261($title="",$data="",$type="")
{
	$_ips___x_retval = '';
...
" . (($displayData['threaded_mode_enabled'] == 0) ? ("<div class='topic_controls'>
	...
	<script type='text/javascript'>
	//<![CDATA[
		if( $('watch_opts_menucontent') ){
			$('watch_opts_menucontent').hide();
		}
	//]]>
	</script>") : ("")) . "
...
<!-- skinnote: Posts by ignored users are not hidden, check _ignored -->
	".$this->__f__a5c20f27de5f37d843dbc7fb4f5140ee($forum,$topic,$post_data,$displayData)."") : ("")) . "
<!-- MOD LINKS -->
				...
				<form action=\"" . $this->registry->getClass('output')->formatUrl( $this->registry->getClass('output')->buildUrl( "app=forums&amp;module=moderate&amp;section=multimod&amp;t={$topic['tid']}", 'public','' ), "", "" ) . "\" method=\"post\" id='multimod' class='left'>
					<fieldset>
						...
						<select name=\"mm_id\" class=\"input_select\">
							...
							".$this->__f__3faf20a434028d386e64040f729b778f($forum,$topic,$post_data,$displayData)."						</select>&nbsp;
			...
			<form method=\"post\" id=\"modform2\" action=\"" . $this->registry->getClass('output')->formatUrl( $this->registry->getClass('output')->buildUrl( "", 'public','' ), "", "" ) . "\">
				<fieldset>
					...
					<select name=\"do\" class=\"input_select\" id='topic_moderation'>
						...
						".$this->__f__6a046279e2b6a42dd670a77ef9d57ecd($forum,$topic,$post_data,$displayData)."					</select>&nbsp;
		...
		<form id='filter_form' method=\"post\" action=\"" . $this->registry->getClass('output')->formatUrl( $this->registry->getClass('output')->buildUrl( "", 'public','' ), "", "" ) . "\">
			<fieldset>
				...
				" . ((is_array($this->caches['reputation_levels'])) ? ("
					".$this->__f__82b9f3aef7f7657a65e1a03061c9668d($forum,$topic,$post_data,$displayData)."				") : ("")) . "
				</select>
...
<div id='fast_reply' style='display: {$displayData['fast_reply']}'>
	...
	<div class='general_box alt'>
		<form action=\"" . $this->registry->getClass('output')->formatUrl( $this->registry->getClass('output')->buildUrl( "", 'publicWithApp','' ), "", "" ) . "\" method=\"post\">
			<fieldset>
				<input type=\"hidden\" name=\"app\" value=\"forums\" />
		...
		<div id='topic_active_users' class='active_users'>
			...
			<ol>
				" . ((is_array( $displayData['active_users']['names'] ) AND count( $displayData['active_users']['names'] )) ? ("
					".$this->__f__aab0562cdd82bb0b4b3e51b1a4d99562($forum,$topic,$post_data,$displayData)."				") : ("")) . "
			</ol>
		</div>
	</div>") : ("")) . "
...
function __f__a5c20f27de5f37d843dbc7fb4f5140ee($forum, $topic, $post_data, $displayData)
{
	$_ips___x_retval = '';
		...
		<div class='post_block hentry clear " . (($this->settings['reputation_highlight'] AND $post['post']['rep_points'] >= $this->settings['reputation_highlight']) ? ("rep_highlight") : ("")) . " " . (($post['post']['queued']==1) ? ("moderated") : ("")) . " " . (($this->settings['reputation_enabled']) ? ("with_rep") : ("")) . "' id='post_id_{$post['post']['pid']}'>
			...
			" . (($this->settings['reputation_enabled']) ? ("" . ((!( $this->settings['reputation_protected_groups'] && in_array( $post['author']['member_group_id'], explode( ',', $this->settings['reputation_protected_groups'] ) ) )) ? ("<div class='rep_bar clear right' id='rep_post_{$post['post']['pid']}'>
					" . (($this->settings['reputation_highlight'] AND $post['post']['rep_points'] >= $this->settings['reputation_highlight']) ? ("
						<span class='rep_highlight'>{$this->lang->words['popular_post']}</span>
					") : ("")) . "
				" . (($post['post']['rep_points'] == 0) ? ("
				...
				" . (($this->memberData['_cache']['report_temp']['post_marker']['post'][ $post['post']['pid'] ]['gfx'] > 0) ? ("
					<ul id='post-report-{$post['post']['pid']}_menucontent' class='ipbmenu_content'>
...
function __f__3faf20a434028d386e64040f729b778f($forum, $topic, $post_data, $displayData)
{
	$_ips___x_retval = '';
...
function __f__6a046279e2b6a42dd670a77ef9d57ecd($forum, $topic, $post_data, $displayData)
{
	$_ips___x_retval = '';
...
function __f__82b9f3aef7f7657a65e1a03061c9668d($forum, $topic, $post_data, $displayData)
{
	$_ips___x_retval = '';
...
function __f__aab0562cdd82bb0b4b3e51b1a4d99562($forum, $topic, $post_data, $displayData)
{
	$_ips___x_retval = '';
