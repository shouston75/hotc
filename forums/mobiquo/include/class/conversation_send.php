...
class mobi_members_messaging_send extends ipsCommand
{
    /**
     * Page title
     *
    ...
    public function doExecute( ipsRegistry $registry )
    {
        ...
        switch( $this->request['do'] )
        {
            ...
            case 'replyForm':
            	...
            	{
            		$result = $this->_showForm2( 'reply' );
            	}
                else 
                {
                	$result = $this->_showForm( 'reply' );
                }
            break;
            case 'send':
                $result = $this->_sendNewPersonalTopic();
            break;
            case 'sendReply':
                $result = $this->_sendReply();
            break;
            case 'editMessage':
    ...
    protected function _sendNewPersonalTopic()
    {
        ...
        if ( !$this->messengerFunctions->withinPMQuota( $this->memberData ) )
        {
            ...
            get_error('cannot_send_new_limit');
        }
        ...
        $msgContent   = to_local(str_replace( "\n", '<br />', $_POST['Post']));
        ...
        if ( IPSText::mbstrlen( trim( $msgTitle ) ) < 2 )
        {
            ...
            get_error($this->lang->words['err_no_title']);
        }
        ...
        if ( !$sendToID AND !$sendToName )
        {
            ...
            get_error($this->lang->words['err_no_chosen_member']);
        }
        ...
        try
        {
            $this->messengerFunctions->sendNewPersonalTopic( $toMember['member_id'], $this->memberData['member_id'], $inviteUsers, $msgTitle, $msgContent, array(
                'isDraft'  => $draft,
                'topicID'  => $topicID,
                'sendMode' => $sendType,
        ...
        return isset($GLOBALS['new_conv_id']) ? $GLOBALS['new_conv_id'] : true;
    }
	...
	protected function _showForm2( $type, $errors='' )
	{
		//-----------------------------------------
		// INIT
		//-----------------------------------------
		...
		if ( $msgID AND ! $_POST['Post-NS'] )
		{
			...
			if ( $msgData['msg_post'] )
			{
				if ( $type == 'reply' )
				{
					...
					/* We're quoting a post... */
					$displayData['message'] = $parser->buildQuoteTag( $msgData['msg_post'], $msgData['members_display_name'], $msgData['msg_date'], 0, 0 );
				}
				else if ( $type == 'edit' )
				{
					/* Reset post key */
		...
		$displayData['uploadData']['attach_stats']	= $class_attach->attach_stats;
 		preg_match('/<textarea id="(.*)" name="msgContent" class=\'ipsEditor_textarea input_text\'>(.*?)<\/textarea>/sie', $displayData['editor'],$match);
 		$displayData['message'] = $match[2];
 		return $displayData;
 	}
