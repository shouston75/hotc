...
|| #################################################################### ||
\*======================================================================*/
require_once( "mobi_classPost.php" );

class mobi_classPostForms extends mobi_classPost
{	
	/**
	 * Constructor
	 *
	...
	protected function _displayForm( $formType, $extraData=array() )
	{
		...
		$output .= $this->registry->getClass('output')->getTemplate('post')->postFormTemplate( array( 'title'            => $titleText,
																									  ...
																									  'postIconSelected' => $this->_generatePostIcons( $post['icon_id'] ),
	...
	public function get_edit_post()
	{
		...
		$postContent = preg_replace('/<img src=\'(.*?)\' class=\'bbc_emoticon\' alt=\'(.*?)\' \/>/si', '$2', $postContent);
		...
		/*
		if (file_exists(IPS_ROOT_PATH . 'sources/classes/editor/composite.php'))
		{
			...
			$postContent = $editor->show('Post', array( 'isHtml' => 1, 'isRte' => 1 ), $postContent);
		}
		*/
		
		$postContent = preg_replace('/^.*?<textarea [^>]*>(.*?)<\/textarea>.*?$/si', '$1', $postContent);
		$postContent = preg_replace('/<br\s*\/>/si', '', $postContent);
	...
	{
	    $quoteContent = $this->_checkMultiQuote("");
	    $quoteContent = str_replace('/<#EMO_DIR#>/', '/#EMO_DIR#/', $quoteContent);
	    $quoteContent = preg_replace('/<img [^>]*?class=(\'|")bbc_emoticon\1 [^>]*?alt=(\'|")(.*?)\2[^>]*?\/>/si', '$3', $quoteContent);
	    
		return $quoteContent;
	}
	
	public function get_title($formType)
	{
		switch( $formType )
		{
			default:
