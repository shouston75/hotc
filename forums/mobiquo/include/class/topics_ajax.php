...
class mobi_forums_ajax_topics extends ipsAjaxCommand
{
    /**
     * Class entry point
     *
    ...
    public function doExecute( ipsRegistry $registry )
    {
		...
		{ 
			...
			{
				case 'get_quote_post':
					$this->request['do'] = "quote";
					$this->request['p'] = $request_params[0];
    ...
    protected function _reply()
    {
        ...
        try
        {
            ...
            /* Mark topic as read */
            $this->registry->getClass('classItemMarking')->markRead( array( 'forumID' => $topic['forum_id'], 'itemID' => $topic['tid'] ) );
    ...
    public function saveTopicTitle()
    {
        ...
        if( ! $tid )
        {
            ...
            get_error($this->lang->words['ajax_no_topic_id']);
        }
        ...
        if( ! $topic['tid'] )
        {
            ...
            get_error($this->lang->words['ajax_topic_not_found']);
        }
        ...
        if( ! $can_edit )
        {
            ...
            get_error($this->lang->words['ajax_no_t_permission']);
        }
        ...
        if( trim( $name ) == '' || ! $name )
        {
            ...
            //exit();
            get_error($this->lang->words['ajax_no_t_name']);
        }
