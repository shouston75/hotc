...
$moderate = new mobi_forums_moderate_moderate($registry);
$moderate->makeRegistryShortcuts($registry);
$result = $moderate->doExecute($registry);


class mobi_forums_moderate_moderate extends ipsCommand
{
    public $output;
    public $modLibrary;
    ...
    {
        // for compatibility of 3.2 approve feature
        global $app_version, $request_name, $request_params;
        if (version_compare($app_version, '3.3.0', '<') && version_compare($app_version, '3.2.0', '>='))
        {
            if ($request_name == 'm_approve_topic' || $request_name == 'm_approve_post') {
                if ($request_params[1] == 2) {
                    $this->request['tact'] = 'unapprove';
                } else {
                    $this->request['tact'] = 'approve';
                    $this->request['do'] = 'topicchoice';
                    $this->request['selectedtids'] = $this->request['t'];
                }
            }
        }
    ...
    /**
     * Permanently remove a hard deleted post
     *
     * @return    @e void
     */
    ...
    protected function _topicApproveAlter( $type='approve' )
    {
        ...
        if( $approve_int )
        {
            ...
            $this->DB->build( array(
                                    ...
                                    'from'     => array( 'posts' => 'p' ),
                                    'order'    => 'pid ASC',
                                    'where'    => $_queued . ' AND p.topic_id=' . $this->topic['tid'],
                                    'add_join' => array(
                                                        array(
    ...
    protected function _deletePost()
    {
        ...
        if ( !$post['pid'] )
        {
            ...
            get_error('mod_no_delete_post_find');
        }
        ...
        if ( $post['new_topic'] == 1 )
        {
            ...
            get_error('mod_delete_first_post');
        }
    ...
    protected function _multiTopicMerge()
    {
        ...
        if ( count($this->tids) < 2 )
        {
            ...
            get_error('mod_topics_merge_two');
        }
        ...
        elseif (count($polls) > 1 )
        {
            ...
            else
            {
                ...
                get_error('Can you merge topics with more than one polls');
            }
        }
        ...
        if ( ! in_array( $this->request['do'], $not_forum_array ) AND ! $this->fromSearch )
        {
            if ( $this->request['p'] )
            {
                ...
                if ( !$this->request['p'] )
                {
                    ...
                    get_error('mod_bad_pid');
                }
                else
                {
                    $this->post = $this->DB->buildAndFetch( array( 'select' => '*', 'from' => 'posts', 'where' => 'pid=' . $this->request['p'] ) );
                    ...
                    if ( !$this->request['t'] ) $this->request['t'] = $this->post['topic_id'];
                }
            }
            ...
            if ( $this->request['t'] )
            {
                ...
                else
                {
                    ...
                    if ( !$this->request['f'] ) $this->request['f'] = $this->topic['forum_id'];
                }
            }
