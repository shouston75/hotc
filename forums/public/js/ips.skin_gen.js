...
var skingen = {
	...
	buildEditor: function(){
		...
		// Make draggable
		new Draggable( 'skingen_editor', { handle: $('skingen_editor').down('h1'), onEnd: this.updateDragPos.bind(this), onStart: this.hidePickers.bind(this) } );
	...
	//------------------------------------------
	// Hides all open color pickers (used when window is dragged)
	//------------------------------------------
	hidePickers: function(e){
		this.skinClasses.each( function(item){
			...
			$H( item.value[3] ).each( function(style){
				if( !Object.isUndefined( this.pickers[ item.key ][ style.key ] ) ){
					this.pickers[ item.key ][ style.key ].hidePicker();
				}
			}.bind(this));
		}.bind(this));	
	},
	...
	buildSkinProcess: function(e)
	{
		...
		// I dislike the following parts immensly for being terrible hacks,
		// but right now, it's the most effective way of ensuring some parts of
		// our CSS do not get overwritten by VSE styles.
		...
		output += ".moderated, body .moderated td, .moderated td.altrow, .post_block.moderated,\
	...
	loadStoredValues: function(){
		...
		$H(storedValues).each( function(item){
			var itemObj = this.skinClasses.get( item.key );

			if( !Object.isUndefined( itemObj ) ){
				this.skinClasses.get( item.key )[3] = item.value;
			}
		}.bind(this));
	},
	...
	xray: {
		...
		findMatchingSelectors: function( item ){
			//Debug.write( this._parent.classList );
			...
			var matching = ancestors.find( function(n){
				return this._parent.classList.find( function(m){
					...
					// Bug fix: prototype .match cannot handle psuedo classes
					// We don't need to check them here anyway
					if( m.match(/\:hover/gi) ){
						return false;
					}
