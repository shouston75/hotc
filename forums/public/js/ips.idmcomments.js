...
_comments.prototype.idmcomments = {
	...
	init: function()
	{
		...
		document.observe("dom:loaded", function(){
			$$('.multiquote', '.delete_item', '.edit_post', '.toggle_post').each( function(elem){
				if( elem.hasClassName('multiquote') ){
					elem.observe('click', ipb.idmcomments.toggleMultimod);
				}
				if( elem.hasClassName('delete_item') ){
	...
	*/
	...
	{	
		...
		edit['post'] = $( 'comment_id_' + edit['cid'] ).down('.comment_content');
	...
	*/
	ajaxEditCancel: function(e)
	{
		...
		if( ipb.idmcomments.postcache[ postid ] ){
			$('comment_id_' + postid).down('.comment_content').update( ipb.idmcomments.postcache[ postid ] );
