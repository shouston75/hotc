...
_ibEconomy.prototype.ibEconomy = {
	...
	initEvents: function()
	{
		...
		ipb.delegate.register(".__item_preview", ipb.ibEconomy.itemPreview);
	},
	...
	*/
	...
	/* ------------------------------ */
	/**
	 * Gives preview of topic
	 * 
	 * @param	{event}		e		The event
	*/
	itemPreview: function(e, elem)
	{
		Event.stop(e);
		...
		var type = $(row).readAttribute("data-type");
		var banktype = $(row).readAttribute("bank-type");
		...
		if( $("item_preview_" + id) )
		{
			if( $("item_preview_wrap_" + id).visible() )
			{ 
				new Effect.BlindUp( $("item_preview_wrap_" + id), { duration: 0.3, afterFinish: function(){ $('item_preview_' + id).hide(); } } );
			...
			{
				$('item_preview_' + id).show();
				new Effect.BlindDown( $("item_preview_wrap_" + id), { duration: 0.3 } );
		...
		{
			var url    = ipb.vars['base_url'] + '&app=ibEconomy&module=ajax&secure_key=' + ipb.vars['secure_hash'] + '&section=shopitem&do=preview&itemid=' + id + '&itemtype=' + type + '&banktype=' + banktype;
			...
			new Ajax.Request( 	url,
				 				{
									...
									onSuccess: function(t)
									{
										...
										if( row.tagName == "TR" )
										{
											...
											var newrow = new Element('tr', { 'class': 'preview', 'id': 'item_preview_' + id });
											var newcell = new Element('td', { 'colspan': count } );
											var wrap = new Element('div', { 'id': 'item_preview_wrap_' + id }).hide().update( new Element('div' ) );
