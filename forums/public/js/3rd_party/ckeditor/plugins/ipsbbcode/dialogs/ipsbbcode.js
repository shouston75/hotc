...
(function()
{
	var ipsBbCodeDialog = function( editor, initVal, fromButton )
	{
		var fetchDdContents = function()
		{
			...
			$H(editor.config.IPS_BBCODE).each( function( tag )
			{
				...
				if ( v.tag != 'code' && v.tag != 'php' && v.tag != 'sql' && v.tag != 'html' )
				{
		...
		var bbCodeChanged = function()
		{
			...
			doBBCodeChange( dialog, typeValue );
		};
		
		var doBBCodeChange = function( dialog, typeValue )
		{
		...
		return {
			...
			onShow : function()
			{
				if ( fromButton === true )
				{
					...
					if ( typeof( editor.config._dialog ) != 'undefined' )
					{
						doBBCodeChange( editor.config._dialog, initVal );
					}
				}
			},
			onOk : function()
	...
	$H(CKEDITOR.config.IPS_BBCODE).each( function( tag )
	{
		...
		if ( v.image && v.image != '' )
		{
			eval( "CKEDITOR.dialog.add( 'ipsbbcode_" + v.tag + "', function( editor ) { return ipsBbCodeDialog( editor, '" + v.tag + "', true ); } );" );
		}
	} );
