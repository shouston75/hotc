	...
	public function runTask()
	{
		...
		if ( $this->settings['eco_loans_on'] && is_array( $bankPorts ) && count( $bankPorts ) )
		{
			...
			{	
				...
				if ( $this->settings['eco_loans_apr_cycle'] * 86400 > time() - $lastChange )
				{
					continue;
				}				
		...
		if ( is_array( $needsInterestPeeps ) and count( $needsInterestPeeps ) and in_array('loan', explode(',', $this->settings['eco_interest_restrict_on']) ) )
		{
		...
		$this->class->appendTaskLog( $this->task, $this->lang->words['loan_interest_added'] );
