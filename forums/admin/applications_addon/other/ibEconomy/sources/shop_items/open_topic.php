...
class class_shop_item implements ibEconomy_shop_item
{
	...
	{
		return $this->lang->words['open_topic'];
	}
	...
	public function usingItem($theItem)
	{
		...
		$itemHtml[] = array('text' => $this->lang->words['enter_topic_wish_to_open'], 'inputs' => "<input type='text' size='30' name='topic_id' id='topic_id' />");
	...
	public function useItem($theItem,$myPortItem)
	{
		...
		#topic already open?
		if ( !$returnMe['error'] && $topic['state'] == 'open' )
		{
			$returnMe['error'] = $this->lang->words['topic_already_open'];
		}		
		...
		if ( ! $returnMe['error'] )
		{
			...
			$returnMe['redirect_text'] = $this->lang->words['topic_has_been_opened'];
		}
