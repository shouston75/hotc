...
class class_shop_item implements ibEconomy_shop_item
{
	...
	{
		return $this->lang->words['unpin_topic'];
	}
	...
	public function usingItem($theItem)
	{
		...
		$itemHtml[] = array('text' => $this->lang->words['enter_topic_wish_to_unpin'], 'inputs' => "<input type='text' size='30' name='topic_id' id='topic_id' />");
	...
	public function useItem($theItem,$myPortItem)
	{
		...
		#topic not pinned?
		if ( !$returnMe['error'] && !$topic['pinned'] )
		{
			$returnMe['error'] = $this->lang->words['topic_already_unpinned'];
		}
		...
		if ( ! $returnMe['error'] )
		{
			...
			$returnMe['redirect_text'] = $this->lang->words['topic_has_been_unpinned'];
		}
