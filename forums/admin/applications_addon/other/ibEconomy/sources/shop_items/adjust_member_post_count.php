...
/**
 * (e32) ibEconomy
 * Shop Item: Adjust Member Post Count
 */
...
class class_shop_item implements ibEconomy_shop_item
{
	...
	{
		return $this->lang->words['adjust_post_count'];
	}
	...
	public function extra_settings()
	{
		$itemSettings = array( 0 => array( 'form_type' 	=> 'formsimpleinput',
										   'field' 		=> 'si_min_num',
										   'words' 		=> $this->lang->words['min_num_posts'],
										   'desc' 		=> $this->lang->words['min_num_posts_exp']
										 ),
							   1 => array( 'form_type' 	=> 'formsimpleinput',
										   'field' 		=> 'si_max_num',
										   'words' 		=> $this->lang->words['max_num_posts'],
	...
	public function useItem($theItem,$myPortItem)
	{
		...
		{
			$numPosts	= rand(intval($theItem['si_min_num']), intval($theItem['si_max_num']));
		}
		else
		{
			$numPosts	= ( intval($theItem['si_max_num']) ) ? intval($theItem['si_max_num']) : 10;
		}
		...
		#newPostCount
		$newPostCount = $daMember['posts'] + $numPosts;
		
		#if negative, is it allowed?
		if( !$returnMe['error'] && !$this->settings['eco_shopitems_steal_neg'] && $newPostCount < 0 )
		{
		...
		{
			#use it
			$this->doUseItem($daMember['member_id'], $newPostCount);
			...
			{			
				...
				$this->registry->ecoclass->sendPM($daMember['member_id'] , '', 0, '', 'generic', $msg2send, $title, $sender, $numPosts );			
			}
			
			#finish up
			$this->registry->ecoclass->finishUpItemUse($theItem,$myPortItem,$daMember['members_display_name'] != $this->memberData['members_display_name'] ? $daMember['members_display_name'].' - '.$numPosts : $numPosts);
			
			#add to redirect text
			$returnMe['redirect_text'] = $this->lang->words['post_count_has_been_adjusted_by'].' '.$this->registry->getClass('class_localization')->formatNumber( $numPosts ).$this->lang->words['!'] ;
		}
	...
	{	
		#update post count
		IPSMember::save( $mid, array( 'members' => array( 'posts' => $newPostCount ) ) );		
	}
}