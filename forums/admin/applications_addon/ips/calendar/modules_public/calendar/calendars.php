 ...
 * IP.Board v3.1.4
 * Calendar View
 * Last Updated: $Date: 2010-11-12 17:23:13 -0500 (Fri, 12 Nov 2010) $
 * </pre>
 *
...
class public_calendar_calendar_calendars extends ipsCommand
{
	 ...
	 * @var		array
	 */
	protected $now				= array( 'mday' => '', 'mon' => '', 'year' => '' );
	 ...
	 * @var		array
	 */
	protected $first_day_array	= array();
	 ...
	 * @var		array
	 */
	protected $query_month_cache	= array();
	 ...
	 * @var		array
	 */
	protected $query_bday_cache	= array();
	 ...
	 * @var		array
	 */
	protected $parsed_members		= array();
	 ...
	 * @var		bool
	 */
	protected $can_avoid_queue	= false;
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		if( $this->page_title == "" )
		{
			$this->page_title =  $this->lang->words['page_title'] . ' - ' . $this->settings['board_name'];
		}
		...
		{
			$this->registry->output->addNavigation( $this->lang->words['page_title'], 'app=calendar', 'calendar', 'app=calendar' );
	 ...
	 * @return	void
	 */
	public function initCalendar( $byPassPermError=false )
	{
		...
		if( ! count( $this->caches['calendars'] ) )
		{
			...
			$this->cache->setCache( 'calendars', $_cal_cache, array( 'array' => 1 ) );
		}
		
		/* Get calendar details */
		if( ( isset($this->request['cal_id']) AND $this->request['cal_id'] ) OR ( isset($this->request['calendar_id']) AND $this->request['calendar_id'] ) )
		{
			$this->calendar_id = intval( $this->request['cal_id'] ) ? intval( $this->request['cal_id'] ) : intval( $this->request['calendar_id'] );
		}
		else
		{
			$cal_ids = array_keys( $this->caches['calendars'] );
			$this->calendar_id = $cal_ids[0];
		}
		...
		if ( ! $this->can_read && ! $byPassPermError )
		{
			$this->registry->output->showError( 'cal_no_perm', 1040, null, null, 403 );
		}
		...
		if( isset($this->request['year']) AND $this->request['year'] )
		{
			$this->request['y'] = $this->request['year'];
		}

		/* Set the chosen month and year */
		$this->chosen_month = ( ! isset($this->request['m']) OR ! $this->request['m'] OR ! intval( $this->request['m'] ) ) ? $this->now_date['mon']  : intval( $this->request['m'] );
		$this->chosen_year  = ( ! isset($this->request['y']) OR ! $this->request['y'] OR ! intval( $this->request['y'] ) ) ? $this->now_date['year'] : intval( $this->request['y'] );
	...
	public function calendarEventDelete()
	{
		...
		{
			if( !$this->memberData['member_id'] OR $this->memberData['member_id'] <> $memcheck['event_member_id'] )
			{
	...
	public function calendarShowMonth()
	{
		...
		$this->page_title	= $this->month_words[$this->chosen_month - 1] . ' - ' . $this->settings['board_name'];
	}
	...
	public function calendarEventForm( $type='add' )
	{
		...
		$calendar_id   = $this->request['calendar_id'] ? intval( $this->request['calendar_id'] ) : $this->calendar_id;
		...
		else
		{
			...
			//	$convert_hours = ( $hour_min[0] * 3600 ) + ( $hour_min[1] * 60 ); /* Bug #20521 */
		...
		$event_type = array(
								...
								'dropdown'    => $calendar_jump, //$this->calendar_jump,		By using $this->calendar_jump, the selected='selected' wasn't set.
								'timestuff'   => array(
														'formtype'  => $form_type,
	...
	public function calendarEventSave( $type='add' )
	{
		...
		$event_tz          = floatval( $this->request['event_tz'] );
		$offset            = 0;
		$event_all_day	   = 0;
		...
		if ( ! $this->memberData['member_id'] )
		{
			$this->registry->output->showError( 'calendar_no_guests', 10412, null, null, 403 );
		}
		...
		if ( ! $this->can_post )
		{
			$this->registry->output->showError( 'calendar_no_post_perm', 10413, null, null, 403 );
		}
		...
		IPSText::getTextClass( 'bbcode' )->parsing_section			= 'calendar';
 		...
 		if ( $type == 'add' )
 		{
 			$_eventData = array( 'event_calendar_id' => $event_calendar_id,
	...
	public function calendarShowDay()
    {
		...
		$this->page_title	= $this->month_words[$this->chosen_month - 1] . ' ' . $day . ' - ' . $this->settings['board_name'];
	...
	public function calendarShowEvent()
	{
		...
		if( $event['event_timeset'] AND !$event['event_all_day'] AND !$event['event_recurring'] )
		{
			$set_offset = $this->settings['time_offset'] * 3600;
		}

		$event['event_unix_from']	= $event['event_unix_from'] + $set_offset;
		$event['event_unix_to']		= $event['event_unix_to'] ? $event['event_unix_to'] + $set_offset : 0;
		...
		$this->page_title	= $event['event_title'] . ' - ' . $this->settings['board_name'];
    }
    ...
    public function calendarMakeEventHTML( $event )
	{
		...
		/* Date */
		// Offset is already applied in the generic getCalendarEventsSql() method
		//$set_offset = 0;
		...
		$event['event_unix_from']	= $event['_unix_from'] ? $event['_unix_from'] : $event['event_unix_from'];
		$event['event_unix_to']		= $event['_unix_to'] ? $event['_unix_to'] : $event['event_unix_to'];
	...
	public function calendarGetEventsSQL( $month=0, $year=0, $get_cached=array() )
	{
		...
		if( !isset($this->event_cache[ $month ]) OR ! is_array( $this->event_cache[ $month ] ) )
		{
			...
			while( $r = $this->DB->fetch() )
			{
				...
				{
					$r['_unix_from'] = $r['_unix_from'] - $set_offset;
					$r['_unix_to']   = $r['_unix_to']   - $set_offset;
	 ...
	 */
	protected function calendarGetMonthDropDown( $month=0 )
	{
		$return = "";
	 ...
	 */
	protected function calendarGetYearDropDown( $year=0 )
	{
		$return = "";
	 ...
	 * @return	string	HTML dropdown
	 */
	protected function calendarGetDayDropDown( $day=0 )
	{
		if( $day == "" )
		{
     ...
     * @return	void
     */
	protected function calendarCallRecache()
    {
