 ...
 * IP.Board v3.1.3
 * Calendar Management
 * Last Updated: $LastChangedDate: 2010-10-24 20:11:34 -0400 (Sun, 24 Oct 2010) $
 * </pre>
 *
...
class admin_calendar_calendar_calendars extends ipsCommand 
{
	...
	public function calendarForm( $type='new' )
	{
		...
		/* Form Elements */
		$form['cal_title']           = $this->registry->output->formInput(        'cal_title'           ,( isset( $_POST['cal_title'] ) AND $_POST['cal_title'] ) ? $_POST['cal_title'] : $calendar['cal_title'] );
	...
	{
		/* Hack to make sure it's installed */
					...
					{ 
						if ( ! in_array( $event['event_id'], $seenids ) )
						{
							...
							if ( $calendar->getInfoEvents( $event, $month, $day, $year, 0 ) )
							{
								...
								/* Set the timezone to the board default */
								//$event['event_unix_from'] = $event['event_unix_from'] + ( $this->settings['time_offset'] * 3600 );
								//$event['event_unix_to'] = $event['event_unix_to'] + ( $this->settings['time_offset'] * 3600 );
	...
	public function calendarRebuildCache( $return=0 )
	{
		...
		$this->cache->setCache( 'calendar' , $calevents, array( 'array' => 1 ) );
	...
	public function calendarsRebuildCache()
	{
		...
		/* Save */
		$this->cache->setCache( 'calendars', $cache, array( 'donow' => 1, 'array' => 1 ) );
