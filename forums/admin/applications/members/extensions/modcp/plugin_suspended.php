<?php
/**
 * @file		plugin_suspended.php 	Moderator control panel plugin: temporarily suspended members
 *~TERABYTE_DOC_READY~
...
/**
 *
 * @class		plugin_members_suspended
 * @brief		Moderator control panel plugin: temporarily suspended members
 * 
 */
class plugin_members_suspended
{
	...
	public function executePlugin( $permissions )
	{
		...
		$this->DB->build( array(
								...
								'order'		=> 'm.joined DESC',
								'limit'		=> array( $st, 10 ),
								'where'		=> "m.temp_ban!=0 AND m.temp_ban!='' AND m.temp_ban " . $this->DB->buildIsNull( false ),
								'add_join'	=> array(
													array(
														'select'	=> 'pp.*',
		...
		$pages	= $this->registry->output->generatePagination( array(	'totalItems'		=> $total['members'],
																		...
																		'baseUrl'			=> "app=core&amp;module=modcp&amp;fromapp=members&amp;tab=suspended",
															  )		);
