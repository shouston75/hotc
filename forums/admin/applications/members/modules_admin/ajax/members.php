 ...
 * Last Updated: $Date: 2012-10-26 11:50:52 -0400 (Fri, 26 Oct 2012) $
 * </pre>
 *
...
class admin_members_ajax_members extends ipsAjaxCommand 
{
	...
	protected function _unSpam()
	{
		...
		if ( $this->request['type'] == 'unspam' OR $this->request['type'] == 'unspam_posts' )
		{
			try
			{
				$message	= $this->_getManagementClass()->unmarkMembersAsSpam( $ids, ( $this->request['type'] == 'unspam' OR $this->request['type'] == 'unspam_posts' ) ? true : false );
			}
			catch( Exception $error )
			{
				$this->returnJsonError( $error->getMessage() );
