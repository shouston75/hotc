 ...
 * Last Updated: $Date: 2012-12-11 17:32:08 -0500 (Tue, 11 Dec 2012) $
 * </pre>
 *
...
class admin_members_bulkmail_bulkmail extends ipsCommand
{
	...
	public function mailSendProcess()
	{
		...
		while ( $r = $this->DB->fetch( $e ) )
		{
			...
			IPSText::getTextClass('email')->buildMessage( array(), false, false, $r );
	...
	protected function _mailSendMandrill()
	{
		...
		while ( $r = $this->DB->fetch( $e ) )
		{
			if ( !$r['email'] or !$r['members_display_name'] )
			{
				continue;
			}
		...
		{
			IPSText::getTextClass('email')->setPlainTextTemplate( $content );
		}
		...
		{
			$content = IPSText::getTextClass('email')->getPlainTextContent();
		}
	...
	protected function _mailSave()
	{
		...
		/* Filters */
		$opts = array( 'mail_html_on' => $_POST['mail_html_on'], 'filters' => array(), 'mandrill_tags' => array_map( 'trim', explode( ',', $this->request['mandrill_tags'] ) ) );
	...
	{
		$queryData = array( 'add_join' => array(), 'where' => array( "m.allow_admin_mails=1" ) );
