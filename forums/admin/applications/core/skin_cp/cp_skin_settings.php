 ...
 * Last Updated: $Date: 2012-11-28 09:38:19 -0500 (Wed, 28 Nov 2012) $
 *
...
<script type='text/javascript'>
	jQ("#settings_table").ipsSortable( 'table', { 
		...
		start: function (e, ui) {
	            var checked= {};
	            jQ('input[type="radio"]', this).each(function(){
	                if(jQ(this).is(':checked'))
	                    checked[jQuery(this).attr('id')] = jQ(this).val();
	                jQ(document).data('checked', checked);
	            });
	        }
	    }).bind('sortstop', function (event, ui) {
	        var restore = jQ(document).data('checked');
