 ...
 * Last Updated: $Date: 2012-12-10 05:08:20 -0500 (Mon, 10 Dec 2012) $
 * </pre>
 *
 * @author 		$Author: mark $
 ...
 *
 * @param	array 		$enhancements		Enhancements data
 * @return	@e string	HTML
 */
public function communityEnhancements( $enhancements=array() )
{
$HTML = '';
...
<div class='section_title'>
	<h2>{$this->lang->words['community_enhancements']}</h2>
</div>

<div class='acp-box'>
	<h3>{$this->lang->words['community_enhancements']}</h3>
	<table class='ipsTable'>
EOF;
	foreach ( $enhancements as $k => $e )
	{
		$external = $e->external ? "target='_blank'" : '';
		$icon = $e->enabled ? 'tick.png' : 'cross.png';
		$HTML .= <<<EOF
		<tr class='{$class}'>
			<td style='width:16px'>
				<img src='{$this->settings['skin_acp_url']}/images/icons/{$icon}' alt='' />
			</td>
			<td>
				<span class='larger_text'><a href='{$this->settings['base_url']}module=applications&amp;section=enhancements&amp;do=edit&amp;service={$k}' {$external}>{$e->title}</a></span><br />
				<span class='desctext'>{$e->description}</span>
			</td>
		</tr>
EOF;
	}
...
/**
 * Community Enhancements - Generic Settings Management
 *
 * @param	stdClass	$class 		Enhancement class
 * @param	array 		$html		HTML
 * @return	@e string	HTML
 */
public function communityEnhancementsSettings( $class, $html )
{
$HTML = '';
...
<form action='{$this->settings['base_url']}module=applications&amp;section=enhancements&amp;do=save&amp;service={$className}' method='POST'>
	<div class='acp-box'>
		<h3>{$class->title}</h3>
		<table class='ipsTable'>
			{$html}
 ...
 * VigLink Conversion
 *
 * @param	string 		$url		Convert URL
 * @return	@e string	HTML
 */
public function viglinkConvert( $url )
{
$HTML = '';

$baseUrl = str_replace( '&amp;', '&', $this->settings['base_url'] );

$HTML .= <<<EOF

<div class='section_title'>
	<h2>{$this->lang->words['enhancements_viglink']}</h2>
</div>

<div class='acp-box'>
	<h3>{$this->lang->words['viglink_signup']}</h3>
	<table class='ipsTable'>
		<tr>
			<td>{$this->lang->words['viglink_blurb']}</td>
		</tr>
	</table>
	<div class='acp-actionbar'>
		<a class='button' href='{$url}' target='_blank' onclick='setTimeout("activate()",1250);'>{$this->lang->words['viglink_signup']}</a>
	</div>
</div>
<br />
<br />

<form action='{$baseUrl}module=applications&section=enhancements&do=save&service=enhancements_core_viglink' method='post'>
	<input type='hidden' name='viglink_manual' value='1' />
	<div class='acp-box'>
		<h3>{$this->lang->words['viglink_existing']}</h3>
		<table class='ipsTable double_pad'>
			<tr>
				<td class='field_title'><strong class='field_field'>{$this->lang->words['viglink_existing_title']}</strong></td>
				<td class='field_field'>{$this->registry->output->formInput( 'viglink_api_key' )}</td>
			</tr>
		</table>
		<div class='acp-actionbar'>
			<input type='submit' class='button' value='{$this->lang->words['viglink_existing_save']}' />
		</div>
	</div>
</form>
<br />
...
<script type='text/javascript'>
	function activate()
	{
		window.location = "{$baseUrl}module=applications&section=enhancements&do=save&service=enhancements_core_viglink";
	}
</script>
 ...
 * IPS CDN Not Enabled
 *
 * @param	array 		$json		Data from license server
 * @return	@e string	HTML
 */
public function cdnNotEnabled( $json )
{
$HTML = '';
...
<div class='section_title'>
	<h2>{$this->lang->words['enhancements_ipscdn']}</h2>
</div>

<div class='acp-box'>
	<h3>{$this->lang->words['enhancements_ipscdn']}</h3>
	<table class='ipsTable'>
		<tr>
			<td>{$this->lang->words['cdn_blurb']}</td>
		</tr>
	</table>
	<div class='acp-actionbar'>
		<a class='button' href='{$json['ACTIVATE_URL']}' target='_blank' onclick='setTimeout("activate()",10000);'>{$this->lang->words['cdn_activate']}</a>
	</div>
</div>
 ...
 * @return	@e string	HTML
 */
public function cdnOverview( $json )
{
$HTML = '';

if ( $json['BYTES'] >= 1099511627776 )
{
	$credit = round( ( $json['BYTES'] / 1099511627776 ), 2 ) . 'TB';
}
else
{
	$credit = round( ( $json['BYTES'] / 1073741824 ), 2 ) . 'GB';
}

$message = sprintf( $this->lang->words['cdn_active'], $credit );
...
<div class='ipsActionBar clearfix'>
	<ul>
		<li class='ipsActionButton'>
			<a href='{$json['MANAGE_URL']}' target='_blank'><img src='{$this->settings['skin_acp_url']}/images/icons/add.png' alt='' /> {$this->lang->words['cdn_manage']}</a>
		</li>
		<li class='ipsActionButton'>
			<a href='{$this->settings['base_url']}module=applications&section=enhancements&do=edit&service=enhancements_core_ipscdn&recache=1'><img src='{$this->settings['skin_acp_url']}/images/icons/arrow_refresh.png' alt='' /> {$this->lang->words['cdn_recache']}</a>
		</li>
		<li class='ipsActionButton'>
			<a href='{$this->settings['base_url']}module=applications&section=enhancements&do=edit&service=enhancements_core_ipscdn&disable=1'><img src='{$this->settings['skin_acp_url']}/images/icons/cross.png' alt='' /> {$this->lang->words['cdn_disable']}</a>
		</li>
	</ul>
</div>
...
$HTML .= @file_get_contents( "http://www.invisionpower.com/clients/interface/ipscdnstats.php?key={$this->settings['ipb_reg_number']}" );
 ...
 * @return	@e string	HTML
 */
public function windowsLiveSetup( $settings )
{

$step1 = sprintf( $this->lang->words['wl_1'], "<a href='http://go.microsoft.com/fwlink/?LinkID=144070' target='_blank'>{$this->lang->words['wl_1b']}</a>", "{$this->settings['board_url']}/interface/board/live.php" );

$form['client_id'] = $this->registry->output->formInput( 'client_id', $settings['client_id'] );
$form['client_secret'] = $this->registry->output->formInput( 'client_secret', $settings['client_secret'] );
$form['xml'] = $this->registry->output->formInput( 'xml', $settings['xml'], '', 80 );
...
<div class='section_title'>
	<h2>{$this->lang->words['enhancements_windowslive']}</h2>
</div>
...
<div class='ipsActionBar clearfix'>
	<ul>
		<li class='ipsActionButton'>
			<a href='{$this->settings['base_url']}module=applications&section=enhancements&do=save&service=enhancements_core_windowslive&disable=1'><img src='{$this->settings['skin_acp_url']}/images/icons/cross.png' alt='' /> {$this->lang->words['wl_disable']}</a>
		</li>
	</ul>
</div>
EOF;
...
$HTML .= <<<EOF
<div class='information-box'>{$step1}</div>
<br />
<form action='{$this->settings['base_url']}module=applications&amp;section=enhancements&amp;do=edit&amp;service=enhancements_core_windowslive&amp;downloadxml=1' method='post'>
	<div class='acp-box'>
		<h3>{$this->lang->words['wl_2']}</h3>
		<table class='ipsTable'>
			<tr>
				<td class='field_title'>
					<strong class='title'>{$this->lang->words['wl_2b']}</strong>
				</td>
				<td class='field_field'>
					{$form['client_id']}
				</td>
			...
			<tr>
				<td class='field_title'>
					<strong class='title'>{$this->lang->words['wl_2c']}</strong>
				</td>
				<td class='field_field'>
					{$form['client_secret']}
				</td>
		...
		<div class='acp-actionbar'>
			<input type='submit' value='{$this->lang->words['wl_2d']}' class='realbutton' />
		</div>
	</div>
</form>
<br />

<div class='information-box'>{$this->lang->words['wl_3']}</div>
<br />
<form action='{$this->settings['base_url']}module=applications&amp;section=enhancements&amp;do=save&amp;service=enhancements_core_windowslive' method='post'>
	<div class='acp-box'>
		<h3>{$this->lang->words['wl_3b']}</h3>
		<table class='ipsTable'>
			<tr>
				<td class='field_title'>
					<strong class='title'>{$this->lang->words['wl_3b']}</strong>
				</td>
				<td class='field_field'>
					{$form['xml']}
				</td>
		...
		<div class='acp-actionbar'>
			<input type='submit' value='{$this->lang->words['wl_3c']}' class='realbutton' />
		</div>
	</div>
</form>
