 ...
 * Last Updated: $Date: 2012-10-15 18:49:44 -0400 (Mon, 15 Oct 2012) $
 * </pre>
 *
...
$HTML .= <<<HTML
			<tr class='language_editor ipsControlRow'>
				<td>
					<div class='information-box' style='width:400px; overflow:auto' id ='word_ib_{$l['id']}_new'>
						<h4>{$l['key']}</h4>
						{$l['default']}
					</div>
					{$pack}
				...
				<td>
					...
					<script type='text/javascript'>
						$('word_{$l['id']}_new').setStyle({'height':($('word_ib_{$l['id']}_new').scrollHeight)+'px'});
					</script>
...
else
{
	$HTML .= <<<HTML
			...
			<td colspan='3' class='no_messages'>
				{$this->lang->words['language_word_pack_no_results']}
			</td>
			</tr>
HTML;
} 	
