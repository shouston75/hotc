 ...
 * Last Updated: $Date: 2012-11-12 10:29:39 -0500 (Mon, 12 Nov 2012) $
 * </pre>
 *
...
EOF;
	...
	{	
		...
		if( is_array( $parents ) AND count( $parents ) )
		{
			...
			{
				if( is_array( $children[ $pid ] ) AND count( $children[ $pid ] ) )
				{
 ...
 *
 * @param	bool		Directory is writable?
 * @param	array 		System Templates
 * @return	string		HTML
 */
public function systemTemplatesList( $writable, $templates ) {
...
$IPBHTML .= <<<EOF
<div class='section_title'>
     <h2>{$this->lang->words['system_templates']}</h2>
</div>
EOF;
...
$IPBHTML .= <<<EOF
<div class='error'>
    {$this->lang->words['system_templates_nowrite']}
</div>
<br />
EOF;
...
$IPBHTML .= <<<EOF
<div class='acp-box'>
	<h3>{$this->lang->words['system_templates']}</h3>
	<table class='ipsTable'>
EOF;
	
	foreach ( $templates as $k => $v )
	{
		$name = isset( $this->lang->words[ 'st_' . $k ] ) ? $this->lang->words[ 'st_' . $k ] : $v;
		$desc = isset( $this->lang->words[ 'st_' . $k . '_desc' ] ) ? ( "<br /><span class='desctext'>" . $this->lang->words[ 'st_' . $k . '_desc' ] . '</span>' ) : '';
	
		$IPBHTML .= <<<EOF
		<tr class='ipsControlRow'>
			<td>
				<span class='larger_text'><a href='{$this->settings['base_url']}&module=templates&section=systemTemplates&do=edit&k={$k}'>{$name}</a></span>{$desc}
			</td>
			<td>
				<ul class='ipsControlStrip'>
					<li class='i_view'><a class='clickable' onclick='window.open( "{$this->settings['base_url']}&module=templates&section=systemTemplates&do=preview&k={$k}", "previewPopup", "height=500,width=800" )' title='{$this->lang->words['preview']}'>{$this->lang->words['preview']}</a></li>
					<li class='i_edit'><a href='{$this->settings['base_url']}&module=templates&section=systemTemplates&do=edit&k={$k}' title='{$this->lang->words['edit']}'>{$this->lang->words['edit']}</a></li>
					<li class='i_revert'><a href='{$this->settings['base_url']}&module=templates&section=systemTemplates&do=revert&k={$k}' title='{$this->lang->words['revert']}'>{$this->lang->words['revert']}</a></li>
				</ul>
			</td>
		</tr>	
EOF;
 ...
 * @return	string		HTML
 */
public function systemTemplatesEdit( $key, $name, $parameters, $content ) {
...
$IPBHTML .= <<<EOF
<form action='{$this->settings['base_url']}&module=templates&section=systemTemplates&do=save&k={$key}' method='post'>
	<div class='acp-box'>
		<h3>{$parameters}</h3>
		<table class='ipsTable'>
			<tr>
				<td>
					<textarea style="width: 100%; height: 800px" name='content'>{$content}</textarea>
				</td>
			</tr>
		</table>
		<div class='acp-actionbar'>
			<input type='submit' value='{$this->lang->words['system_templates_save']}' class='realbutton' />
		</div>
	</div>
</form>
EOF;
