...
<div class='acp-box with_bg'>
	...
	<p><b><a href='{$this->settings['base_url']}{$this->form_code}&amp;do=dosafebackup&amp;create_tbl={$this->request['create_tbl']}&amp;skip={$this->request['skip']}&amp;enable_gzip={$this->request['enable_gzip']}'>{$this->lang->words['my_start_backup']}</a></b></p>
</div>
HTML;
 ...
 * @return	string		HTML
 */
public function sqlViewResults( $queries, $headers, $rows, $pages, $truncated=array() ) {
$IPBHTML = "";
//--starthtml--//
...
{	
	if ( $aQuery['tableName'] )
	{
		$query_title = $this->lang->words['my_results'] . ' - ' . $aQuery['tableName'];
	}
	else
	{
		$query_title = $this->lang->words['my_results'];
	}
