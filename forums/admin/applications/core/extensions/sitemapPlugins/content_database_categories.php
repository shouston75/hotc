...
class sitemap_core_content_database_categories extends ipseoSitemapPlugin
{
	public function generate()
	{
		...
		$permCheck1 = $this->DB->buildWherePermission(array($this->settings['guest_group']), 'perm1.perm_view', true);
		$permCheck2 = $this->DB->buildWherePermission(array($this->settings['guest_group']), 'perm2.perm_view', true);
		...
		$this->DB->build(array(	'select'	=> 'c.category_id, c.category_last_record_date, d.database_id, p.*',
								...
								'where'		=> "	{$permCheck1} AND 
													(perm2.perm_view IS NULL OR perm2.perm_view = '' OR {$permCheck2}) AND
													({$permCheck3})",
