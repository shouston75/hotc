	...
	*/
	public function generate()
	{
		...
		$this->DB->build( array(
			...
			'where'		=> $this->DB->buildWherePermission( array( $this->settings['guest_group'] ), 'p.perm_view' )
			) );
		$this->DB->execute();
		...
		$this->DB->build( array(
			...
			'from'		=> 'cal_events',
			'where'		=> "event_end_date < {$time} AND " . $this->DB->buildWherePermission( $calendars, 'event_calendar_id', FALSE ) . ' AND ' . $this->DB->buildWherePermission( array( $this->settings['guest_group'] ), 'event_perms' ),
