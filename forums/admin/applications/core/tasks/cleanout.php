 ...
 * $Author: mark $
 * @since		-
 * $LastChangedDate: 2012-06-28 06:20:34 -0400 (Thu, 28 Jun 2012) $
 * @version		v3.4.1
 * $Revision: 11000 $
 */
...
class task_item
{
	...
	public function runTask()
	{
		...
		//-----------------------------------------
		// Delete old inline messages
		//-----------------------------------------
		
		$this->DB->delete( 'core_inline_messages', 'inline_msg_date < ' . ( IPS_UNIX_TIME_NOW - 300 ) );
		
		//-----------------------------------------
		// Delete old search engine visit logs
		//-----------------------------------------
						
		ipsRegistry::DB()->delete('search_visitors', 'date < ' . ( IPS_UNIX_TIME_NOW - ( 86400 * 35 ) ) );
