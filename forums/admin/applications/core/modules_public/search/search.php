 ...
 * IP.Board v3.4.1
 * Global Search
 * Last Updated: $Date: 2012-10-18 17:53:54 -0400 (Thu, 18 Oct 2012) $
 * </pre>
 *
...
class public_core_search_search extends ipsCommand
{
	...
	/**
	 * Clean search keys and values
	 */
	protected function _cleanInputFilters( $filters )
	{
		if ( is_array( $filters ) )
		{
			foreach( $filters as $k => $v )
			{
				...
				$k = IPSText::alphanumericalClean( $k );
				
				if ( is_array( $v ) )
				{
					$filters[ $k ] = $this->_cleanInputFilters( $v );
				}
				else
				{
					$filters[ $k ] = IPSText::htmlspecialchars( $v );
				}
			}
		}
