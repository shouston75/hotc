 ...
 * IP.Board v3.4.1
 * Register
 * Last Updated: $Date: 2012-11-28 05:08:49 -0500 (Wed, 28 Nov 2012) $
 * </pre>
 *
 * @author 		$Author $
				...
				if ( !empty( $chosenPackages ) )
				{
					foreach ( $chosenPackages as $itemID )
					{
						...
						$items[] = array(
							'act'			=> 'new',
							'app'			=> 'nexus',
							'type'			=> $package->data['p_type'],
							'cost'			=> $package->price( $this->memberData['member_id'], FALSE ),
							'tax'			=> $package->data['p_tax'],
							...
							'itemName'		=> $package->data['p_name'],
							'itemID'		=> $package->data['p_id'],
							'groupRenewals'	=> $package->data['p_group_renewals'],
							'methods'		=> $package->data['p_methods'] ? explode( ',', $package->data['p_methods'] ) : array(),
							);
					}
