...
class public_core_global_unsubscribe extends ipsCommand
{
	...
	{	
		...
		if ( $member['member_id'] and $this->request['key'] == md5( $member['email'] . ':' . $member['members_pass_hash'] ) )
		{
			...
			$this->registry->output->redirectScreen( $this->lang->words['email_unsubscribed'], $this->settings['board_url'] );
			return;
		}
						
		$this->registry->output->redirectScreen( $this->lang->words['email_no_unsubscribe'], $this->settings['board_url'] );
    }
}