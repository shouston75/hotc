 ...
 * IP.Board v3.4.1
 * Diagnostic tools
 * Last Updated: $Date: 2012-10-03 10:24:03 -0400 (Wed, 03 Oct 2012) $
 * </pre>
 *
 * @author 		$Author: mark $
...
class admin_core_diagnostics_diagnostics extends ipsCommand
{
	...
	{	
		$checkdirs = array( IPS_PUBLIC_PATH . 'style_images', 
							IPS_PUBLIC_PATH . 'style_css', 
							IPS_PUBLIC_PATH . 'style_emoticons', 
							IPS_CACHE_PATH . 'cache', 
							IPS_CACHE_PATH . 'cache/skin_cache', 
							IPS_CACHE_PATH . 'cache/lang_cache', 
							IPS_CACHE_PATH . 'cache/tmp', 
							$this->settings['upload_dir'],
							$this->settings['upload_dir'] . '/profile', 
							IPS_HOOKS_PATH,
							);
		...
		if( is_array( $this->cache->getCache('lang_data') ) && count( $this->cache->getCache('lang_data') ) )
		{
			...
			{
				$this_lang   = IPS_CACHE_PATH . 'cache/lang_cache/' . $v['lang_id'];
				$checkdirs[] = $this_lang;
				
				try
				{
					foreach( new DirectoryIterator( $this_lang ) as $file )
					{
						if( $file->isFile() )
						{
		...
		if( is_array( $this->cache->getCache('emoticons') ) && count( $this->cache->getCache('emoticons') ) )
		{
			...
			{
				$checkdirs[] = IPS_PUBLIC_PATH . 'style_emoticons/' . $v['emo_set'];
			}
		}
		else
		{
		...
		if( is_array( $this->cache->getCache('skin_id_cache') ) && count( $this->cache->getCache('skin_id_cache') ) )
		{
			foreach( $this->cache->getCache('skin_id_cache') as $k => $v )
			{
				...
				$checkdirs[]	= IPS_CACHE_PATH . 'cache/skin_cache/cacheid_' . $v['set_id'];
		...
		foreach( $checkdirs as $dir_to_check )
		{
			if( !file_exists( $dir_to_check ) )
			{
				...
				{
					$output[] = "<span style='color:red;font-weight:bold;'>{$this->lang->words['d_p404']} {$dir_to_check}</span>";
				}
			}
			else 
			{
				$output[] = ( is_writable( $dir_to_check ) ) ? "<span style='color:green;'>{$dir_to_check} {$this->lang->words['d_pyes']}</span>" : "<span style='color:red;font-weight:bold;'>{$this->lang->words['d_pno']} {$dir_to_check}</span>";
			}
		}
