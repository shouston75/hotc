 ...
 * IP.Board v3.4.1
 * Hooks Management
 * Last Updated: $Date: 2012-10-18 14:57:47 -0400 (Thu, 18 Oct 2012) $
 * </pre>
 *
...
EOF;
	...
	public function _updateAvailable( $url, $version )
	{
		...
		/* Replace {IPSID} id necessary */
		if ( strpos( $return[1], '{IPSID}' ) )
		{
			if ( ipsRegistry::$settings['ipb_reg_number'] )
			{
				$exploded = explode( '-', ipsRegistry::$settings['ipb_reg_number'] );
				$return[1] = str_replace( '{IPSID}', $exploded[3], $return[1] );
			}
			else
			{
