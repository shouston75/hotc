 ...
 * IP.Board v3.4.1
 * Live Search
 * Last Updated: $Date: 2012-12-06 10:19:49 -0500 (Thu, 06 Dec 2012) $
 * </pre>
 *
 * @author 		$Author: mark $
	...
	protected function _checkNexus( $term, $results )
	{
		...
		if ( (string) $term == (string) $intTerm )
		{
			...
			if ( $this->registry->getClass('class_permissions')->checkPermission( 'transactions_manage', 'nexus', 'payments' ) )
			{
				...
				if ( $transaction['t_id'] )
				{
					$tempResults['transactions'] = array( 'name' => $transaction['t_id'], 'url' => $this->settings['_base_url']."&amp;app=nexus&amp;module=payments&amp;section=transactions&amp;do=view&amp;id={$transaction['t_id']}", 'img' => $this->settings['base_acp_url'] . '/applications_addon/ips/nexus/skin_cp/images/customers/transaction.png' );
				}
			}
