...
class admin_core_tools_login extends ipsCommand
{
	...
	protected function _loginSaveConf()
	{
		...
		ipsRegistry::getClass('output')->redirect( $this->settings['base_url'] . $this->form_code . "", sprintf($this->lang->words['l_confup'], $login['login_title'] ) );
		return true;
	}
	...
	public function _loginACPConf()
	{
		...
		ipsRegistry::getClass('output')->html .= $this->html->login_conf_form( $login, $form, isset( $help ) ? $help : NULL );
	}
	...
	protected function _loginToggle()
	{
		...
		ipsRegistry::getClass('output')->redirect( $this->settings['base_url'] . $this->form_code . "", $this->lang->words['l_successfully'] . $toggle );
	}
	...
	protected function _loginUninstall()
	{
		...
		ipsRegistry::getClass('output')->redirect( $this->settings['base_url'] . $this->form_code . "", $this->lang->words['l_uninstalled'] );
	}
