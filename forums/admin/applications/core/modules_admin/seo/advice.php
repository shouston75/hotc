...
class admin_core_seo_advice extends ipsCommand
{
	...
	public function dashboard()
	{
		...
		if( !array_key_exists('sitemap_success', $ignores) )
		{
			$sitemapTask = $this->DB->buildAndFetch( array( 'select' => '*', 'from' => 'task_manager', 'where' => "task_application='core' AND task_key='ipseo_sitemap_generator'" ) );
			$lastLog = $this->DB->buildAndFetch( array( 'select' => '*', 'from' => 'task_logs', 'where' => "log_title='{$sitemapTask['task_title']}'", 'order' => 'log_date DESC', 'limit' => 1 ) );
			
			require_once IPS_ROOT_PATH . 'sources/classes/class_taskmanager.php';
			$taskManager = new class_taskmanager( $this->registry );
			$nextRun = $taskManager->generateNextRun( $sitemapTask );
			
			if ( !$lastLog['log_id'] or ( $lastLog['log_date'] < ( time() - ( $nextRun - ( time() + 7200 ) ) ) ) or strpos( $lastLog['log_desc'], $this->lang->words['smt_finished'] ) === FALSE )
			{
				$messages[] = array('level' => 'bad', 'fix' => 'app=core&module=system&section=taskmanager&do=overview', 'key' => 'sitemap_success');
			}
		}
		...
		if(!array_key_exists('seo_index_title', $ignores) && !isset( $metaTags['']['title'] ) )
		{
			$messages[] = array('level' => 'bad', 'fix' => "app=core&module=templates&section=meta", 'key' => 'seo_index_title');
		}
		
		/* No board index description */
		if( !array_key_exists('seo_index_md', $ignores) && !isset( $metaTags['']['description'] ) )
		{
			$messages[] = array('level' => 'bad', 'fix' => "app=core&module=templates&section=meta", 'key' => 'seo_index_md');
		}		
