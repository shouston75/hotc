...
class admin_core_sql_toolbox_module extends ipsCommand {
	...
	public function sqlViewResults( $sql )
	{
		...
		else
		{
			...
			foreach( $the_queries as $key => $sql )
			{
				/* INIT */
				$links 	= "";
				...
				else if ( preg_match( "/^(?!SELECT)/i", preg_replace( "#\s{1,}#s", "", $sql['sql'] ) ) and preg_match( "/admin_login_logs/i", preg_replace( "#\s{1,}#s", "", $sql['sql'] ) ) )
				{
					$columns[ $queryCntForArray ][]	= $this->lang->words['manual_error'];
					...
					if ( ( ! preg_match( "/^EXPLAIN/i", $sql['sql'] ) && ! preg_match( "/^SHOW/i", $sql['sql'] ) ) AND ! preg_match( "/LIMIT[ 0-9,]+$/i", $sql['sql'] ) )
					{
						...
						if( $rows_returned > $limit )
						{
							$links = $this->registry->output->generatePagination( array(
																						...
																						'baseUrl'           => "{$this->settings['base_url']}{$this->form_code}&do=runsql&query=" . urlencode( $sql['sql'] ),
																				)	);
