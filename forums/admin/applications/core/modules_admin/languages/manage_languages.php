 ...
 * Last Updated: $LastChangedDate: 2012-12-11 06:40:23 -0500 (Tue, 11 Dec 2012) $
 * </pre>
 *
...
class admin_core_languages_manage_languages extends ipsCommand
{
	...
	public function swapLanguages()
	{
		...
		if( !$langFrom )
		{
			$this->DB->update( 'members', array( 'language' => $langTo > 0 ? $langTo : 0 ), "language=0 OR language " . $this->DB->buildIsNull( true ) );
		}
		else
		{
			$this->DB->update( 'members', array( 'language' => $langTo > 0 ? $langTo : 0 ), "language=" . intval( $langFrom ) );
		}
	...
	{			
		...
		$lang = $this->DB->buildAndFetchAll( array( 
			'select'	=> '*', 
			'from'		=> 'core_sys_lang_words', 
			'where'		=> "lang_id={$id} AND word_id IN(" . implode( ',', array_map( 'intval', array_keys( $_POST['lang'] ) ) ) . ')',
			), 'word_id' );
