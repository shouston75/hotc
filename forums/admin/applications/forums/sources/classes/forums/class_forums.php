 ...
 * IP.Board v3.4.1
 * Forums Class
 * Last Updated: $Date: 2012-12-03 21:50:28 -0500 (Mon, 03 Dec 2012) $
 * </pre>
 *
...
class class_forums
{
	...
	/**
	 * is the answer system enabled?
	 * @param int $forumId
	 */
	public function answerTopicsEnabled( $forumId )
	{ 
		$forumData = $this->getForumById( $forumId );
	
		return ( ! empty( $forumData['bw_enable_answers'] ) ) ? true : false;
	}
	...
	public function forumsCalcChildren( $root_id, $forum_data=array(), $done_pass=0 )
	{
		...
		/* Let's fix an empty forum ID... Bug #38436 */
		if ( empty($forum_data['fid']) && $forum_data['id'] )
		{
	...
	public function forumRebuild( $fid )
	{
		...
		$last_post		= $this->DB->buildAndFetch( array( 'select'	=> 'tid, title, last_poster_id, last_poster_name, seo_last_name, last_post',
														   'from'	=> 'topics',
														   'where'	=> $this->fetchTopicHiddenQuery( array( 'visible' ), '' ) . " and forum_id={$fid}" . " AND state != 'link'",
		...
		$newest_topic	= $this->DB->buildAndFetch( array( 'select'	=> 'title, tid, seo_first_name',
														   'from'	=> 'topics',
														   'where'	=> 'forum_id=' . $fid . ' and ' . $this->fetchTopicHiddenQuery( array( 'visible' ), '' ) . " AND state != 'link'",
	...
	public function pageToSt( $page )
	{
		...
		return ( $page > 1 ) ? ( ( $page - 1 ) * $this->settings['display_max_topics'] ) : 0;
	}
	...
	public function stToPage( $st )
	{
		...
		return ( $st > 0 ) ? ( ceil( $st / $this->settings['display_max_topics'] ) + 1 ) : 1;
	}
