 ...
 * Last Updated: $Date: 2010-10-19 08:45:58 -0400 (Tue, 19 Oct 2010) $
 * </pre>
 *
...
class threadedModeLibrary
{
	...
	public function _getTopicDataThreaded()
	{
		...
		{
			$this->registry->output->showError( 'topics_post_not_in_topic', 10358, true, null, 404 );
		}
		...
		$_post_joins = array(
								array( 
										'select' => 'm.member_id as mid,m.name,m.member_group_id,m.email,m.joined,m.posts, m.last_visit, m.last_activity,m.login_anonymous,m.title,m.hide_email, m.warn_level, m.warn_lastwarn, m.members_display_name, m.members_seo_name, m.has_gallery, m.has_blog, m.members_bitoptions,m.mgroup_others'.$_extraMember,
		...
		{
			/* That 1 line fix below took 1 hour, joy :D */
			$row['member_id']	= $row['mid'];
			$row['g_access_cp']	= $this->caches['group_cache'][ $row['member_group_id'] ]['g_access_cp'];
		...
		$this->topics->topic['threaded_pages']	= $this->registry->output->generatePagination( array( 
																							...
																							'seoTitle'			=> $this->topics->topic['title_seo']
																					)  	);
	...
	public function _buildThreadedOutput( $root, $html='' )
	{
		...
		$html .= $this->registry->output->getTemplate('topic')->build_threaded( $root, $child, $this->topics->topic );
	...
	private function _threadedPostGetChildren($root_id, $ids=array())
	{
		if ( is_array($this->structured_pids[ $root_id ]) )
		{
			foreach( $this->structured_pids[ $root_id ] as $pid )
			{
				...
				$ids = $this->_threadedPostGetChildren( $pid, $ids );
			}
		}
