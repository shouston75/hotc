 ...
 * IP.Board v3.4.1
 * Moderator actions
 * Last Updated: $Date: 2012-11-30 09:28:39 -0500 (Fri, 30 Nov 2012) $
 * </pre>
...
EOF;
	...
	public function editSetUp()
	{
		...
		if ( ! $this->getAuthor('g_is_supmod') AND empty( $this->moderator['edit_post'] ) )
		{
			$perm_id	= $this->getAuthor('org_perm_id') ? $this->getAuthor('org_perm_id') : $this->getAuthor('g_perm_id');
			$perm_array = explode( ",", $perm_id );
		...
		if ( $this->_originalPost['new_topic'] == 1 )
		{
			...
			else if ( isset($this->moderator['edit_topic']) && $this->moderator['edit_topic'] == 1 )
			{
				$this->edit_title       = 1;
				$this->can_add_poll_mod = 1;
			}
	...
	public function sendSocialShares( array $topicData )
	{
		...
		if ( count( $services ) )
		{
			$data = array( 'title' => $topicData['title'],
						   'url'   => $this->registry->output->buildSEOUrl( 'showtopic=' . $topicData['tid'] . '&amp;view=getnewpost', 'publicNoSession', $topicData['title_seo'], 'showtopic' ) );
	...
	public function sendOutTrackedTopicEmails( $topic, $content="" )
	{
		...
		$poster_url  = $this->registry->output->buildSEOUrl( 'showuser=' . $_poster['member_id'], 'publicNoSession', $_poster['members_seo_name'], 'showuser' );
		$url         = $this->registry->output->buildSEOUrl( 'showtopic=' . $topic['tid'] . '&amp;view=getnewpost', 'publicNoSession', $topic['title_seo'], 'showtopic' );
		...
		{			
			if ( preg_match_all( '#(\[quote|\<blockquote)(?:[^\]]+?)data-cid=(?:\'|"|&quot;|&\#39;)(\d+?)(?:\'|"|&quot;|&\#39;)#', $post['post'], $quotes) )
			{
				...
				foreach( $posts AS $pid => $_quoted )
				{
					...
					$ignored = $this->DB->buildAndFetch( array( 'select' => '*',
							'from'   => 'ignored_users',
							'where'  => 'ignore_ignore_id=' . $this->memberData['member_id'] . ' AND ignore_owner_id=' . $_toMember['member_id'] . ' AND ignore_topics = 1' ) );					
	...
	protected function compilePostData()
	{
		...
		if ( is_array( $this->editor->getParsingErrors() ) && count( $this->editor->getParsingErrors() ) )
		{
	...
	public function formatPost( $postContent )
	{
		...
		$this->editor->setIsHtml( ( intval($this->request['post_htmlstatus']) AND $this->getForumData('use_html') AND $this->getAuthor('g_dohtml') ) ? 1 : 0 );
		$this->editor->setAllowBbcode( $this->getForumData('use_ibc') );
		$this->editor->setAllowSmilies( $this->getSettings('enableEmoticons') );
		$this->editor->setBbcodeSection( 'topics' );
	...
	public function formatPostForCache( $postContent )
	{
		/* Set up parser */
		$parser = new classes_text_parser();
		$parser->set( array( 'memberData'     => $this->getAuthor(),
							 'parseBBCode'    => $this->getForumData('use_ibc'),
							 'parseArea'	  => 'topics',
				  			 'parseHtml'      => (intval($this->request['post_htmlstatus']) AND $this->getForumData('use_html') AND $this->getAuthor('g_dohtml')) ? 1 : 0,
