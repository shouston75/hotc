 ...
 * IP.Board v3.1.4
 * UserCP Tab
 * Last Updated: $Date: 2010-10-28 06:58:47 -0400 (Thu, 28 Oct 2010) $
 * </pre>
 *
...
class usercpForms_forums extends public_core_usercp_manualResolver implements interface_usercp
{
	...
	public function getLinks()
	{
		...
		$array[] = array( 'url'   => 'area=settings',
						  'title' => ipsRegistry::instance()->getClass('class_localization')->words['rboard_prefs'],
	...
	private function _customEvent_modAnnounceForm( $type='add', $msg='' )
	{
		...
		$this->_nav[] = array( ipsRegistry::instance()->getClass('class_localization')->words[ $type == 'add' ? 'announce_add' : 'announce_button_edit' ] );
	...
	private function _customEvent_updateTopics()
	{
		...
		//-----------------------------------------
 		...
 		{
 			$this->registry->getClass('output')->silentRedirect( $this->settings['base_url'] . 'showtopic=' . intval( $this->request['topicReturn'] ) . '&amp;st=' . intval( $this->request['st'] ) );
 		}
 		else
 		{
	...
	private function _customEvent_watch( $saveIt=FALSE )
	{
		...
		{
			$this->registry->getClass('output')->showError( 'usercp_forums_no_perms', 1031, null, null, 403 );
		}
		...
		if ( isset( $forum['password_override'] ) && IPSText::cleanPermString($forum['password_override']) != '' )
		{
			if ( IPSMember::isInGroup( $this->memberData, explode( ",", IPSText::cleanPermString($forum['password_override']) ) ) )
			{
				$deny_access = 0;
			}
		}
		
		if ( $deny_access AND ( isset($forum['password']) AND $forum['password'] != "" ) )
		{
			if ( $this->registry->getClass('class_forums')->forumsComparePassword( $forum['id'] ) != TRUE )
			{
				$this->registry->getClass('output')->showError( 'usercp_forums_must_login', 1032, null, null, 403 );
			}
		}
	...
	public function showTopicSubs()
	{
		...
		$mainForumArray = array();
 		...
 		$forumController = new $classToLoad();
		...
		if( ( $this->settings['show_user_posted'] == 1 ) and ( $this->memberData['member_id'] ) and count($mainForumArray) )
		{
			$this->DB->build( array( 
									...
									'from'   => 'posts',
									'where'  => 'queued=0 AND author_id=' . $this->memberData['member_id'] . ' AND topic_id IN(' . implode( ',', array_keys( $topic_ids ) ) . ')',
							)	);
	...
	public function showForumSettings()
	{
		...
		return $this->registry->getClass('output')->getTemplate('ucp')->forumPrefsForm( array( 
																							'viewTopicsForm'	=> $tp_a,
																							'viewPostsForm'		=> $pp_a,
