...
class post_plugin
{
	...
	public function processReport( $com_dat )
	{
		...
		$data = $this->DB->buildAndFetch( array(
												'select'	=> 'p.post, p.post_date, p.pid',
												'from'		=> array( 'posts' => 'p' ),
												'where'		=> 'p.pid=' . $post_id,
												'add_join'	=> array(
																	array(
		...
		$message = $parser->buildQuoteTag( $data['post'], $data['members_display_name'], $data['post_date'], 0, $data['pid'] );
		...
		else
		{
			...
			$rid		= $the_report['id'];
			$this->DB->update( 'rc_reports_index', array( 'date_updated' => time(), 'status' => $status['new'], 'seoname' => $data['title_seo'], 'seotemplate' => 'showtopic', 'updated_by'	=> $this->memberData['member_id'] ), "id='{$rid}'" );
		}
