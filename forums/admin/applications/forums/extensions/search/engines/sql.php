 ...
 * IP.Board v3.4.1
 * Basic Forum Search
 * Last Updated: $Date: 2012-11-13 04:36:13 -0500 (Tue, 13 Nov 2012) $
 * </pre>
 *
...
class search_engine_forums extends search_engine
{
		...
		if ( $search_tags && $this->settings['tags_enabled'] )
		{
			...
			if ( is_array( $tags) && count( $tags ) )
			{
				...
				$this->DB->build( array('select'	=> "t.tid as id, t.tid as topic_id, t.title, t.posts, t.views, t.last_post, t.topic_firstpost",
										'from'		=> array( 'topics' => 't' ),
		 								'where'		=> 't.tid IN (' . implode( ",", $_tagIds ) . ') AND ' . $this->_buildWhereStatement( $search_term, IPSSearchRegistry::searchTitleOnly() ),
										'limit'		=> array(0, IPSSearchRegistry::get('set.hardLimit') + 1),
	...
	public function viewNewContent()
	{
		...
		if ( $count['count'] )
		{
			$limit = $noLimit ? NULL : ( IPSSearchRegistry::get('in.period') != 'unread' ) ? array( $start, $perPage ) : array( 0, 1200 );
