...
class admin_forums_archive_archive extends ipsCommand
{
	...
	protected function _saveRestorePrefs()
	{
		...
		$mouse = $archiveRestore->engine->remoteDB->buildAndFetch( array( 'select'  => 'min(archive_topic_id) as min, max(archive_topic_id) as max',
												  'from'    => 'forums_archive_posts',
												  'where'   => 'archive_content_date > ' . $date ) ); /* @todo - abstract this to the engine */
	...
	protected function _archiveDash()
	{
		...
		$archTopics = $topicClass->getTopics( array( 'archiveState' => array( 'working', 'archived' ), 'topicType' => array( 'hidden', 'visible' ), 'limit' => 10, 'getCount' => true, 'sortField' => 'tid', 'sortOrder' => 'desc' ) );
