 ...
 * Last Updated: $Date: 2012-12-11 09:44:42 -0500 (Tue, 11 Dec 2012) $
 * </pre>
 *
...
class public_forums_ajax_topics extends ipsAjaxCommand
{
	...
	protected function _postShareModal()
	{
		...
		$this->registry->getClass('topics')->getPosts( array(   'topicId' 		 => $tid,
				                                                'limit'			 => 1,
																'skipForumCheck' => true,
																'pidIsLess'      => $pid,
																'onlyViewable'   => true,
	...
	protected function _reply()
	{
		...
		/* Regular group flood check */		
		if ( $this->settings['flood_control'] > 0 && $this->memberData['g_avoid_flood'] != 1 )
		{
		...
		try
		{
			...
			$topic					= $this->registry->getClass('topics')->setUpTopic( $this->registry->getClass('topics')->getTopicData() );
			...
			else
			{
				$html		= '';
				$lastpid	= 0;
				$forumData = $this->postClass->getForumData();
				...
				foreach( $posts as $pid => $data )
				{
					...
					(
							...
							(
									...
									and
									(
											( $data['approved'] == 0 and $this->memberData['forumsModeratorData'][ $forumData['id'] ]['post_q'] )
											or
											( $data['queued'] == 2 and $this->memberData['forumsModeratorData'][ $forumData['id'] ]['bw_mod_un_soft_delete'] )
											or
											( $data['queued'] == 0 and $this->memberData['forumsModeratorData'][ $forumData['id'] ]['bw_mod_soft_delete'] )
									)
							)
					)
					{
						$data['post']['moddable'] = TRUE;
					}
									
					$html		.= $this->registry->output->getTemplate('topic')->post( $data, array(), $topic, $forumData );
					$lastpid	= $pid;
				}
	...
	public function editBoxSave()
	{
		...
		try
		{
			...
			/* Set up parser */
			$parser = new classes_text_parser();
			$parser->set( array( 'memberData'     => $this->postClass->getAuthor(),
								 'parseBBCode'    => $this->registry->getClass('class_forums')->forum_by_id[ $fid ]['use_ibc'],
								 'parseArea'	  => 'topics',
					  			 'parseHtml'      => ( $this->registry->getClass('class_forums')->forum_by_id[ $fid ]['use_html'] and $this->memberData['g_dohtml'] and $post['post_htmlstate'] ) ? 1 : 0,
