 ...
 * IP.Board v3.4.1
 * Posting
 * Last Updated: $Date: 2012-11-08 03:17:42 -0500 (Thu, 08 Nov 2012) $
 * </pre>
		...
		switch( $type )
		{
			case 'reply':
				try
				{
					...
					{
						ipsRegistry::getClass('output')->silentRedirect( $this->settings['base_url']."showtopic={$topic['tid']}&amp;p={$post['pid']}", $topic['title_seo'] );
					}
				}
				catch( Exception $error )
				{
			...
			case 'edit':
				try
				{
					...
					# Redirect
					ipsRegistry::getClass('output')->redirectScreen( $this->lang->words['post_edited'], $this->settings['base_url'] . "showtopic={$topic['tid']}&st=" . $this->request['st'] . "&p={$post['pid']}", $topic['title_seo'] );
