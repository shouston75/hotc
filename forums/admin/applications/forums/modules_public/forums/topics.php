 ...
 * IP.Board v3.4.1
 * Topic View
 * Last Updated: $Date: 2012-12-06 17:57:19 -0500 (Thu, 06 Dec 2012) $
 * </pre>
 *
...
class public_forums_forums_topics extends ipsCommand
{
	...
	public function parsePostRow( $row = array() )
	{
		...
		else if( $this->request['page'] AND ! $this->_firstPostContent )
		{
			$this->_firstPostContent = $parsed['post']['post'];
		}
	...
	*/
			...
			if ( $voter['member_choices'] AND ( ( $poll['poll_view_voters'] AND $this->settings['poll_allow_public'] ) OR $voter['member_id'] == $this->memberData['member_id'] ) )
			{
				...
				if ( is_array( $_choices ) AND count( $_choices ) )
				{
					$memberData = array( 'member_id'            => $voter['member_id'],
										 'members_seo_name'     => ( $voter['member_id'] == $this->memberData['member_id'] ) ? $this->memberData['members_seo_name'] : $voter['members_seo_name'],
										 'members_display_name' => ( $voter['member_id'] == $this->memberData['member_id'] ) ? $this->memberData['members_display_name'] : $voter['members_display_name'],
										 'members_colored_name' => str_replace( '"', '\"', ( $voter['member_id'] == $this->memberData['member_id'] ) ? IPSMember::makeNameFormatted( $this->memberData['members_display_name'], $this->memberData['member_group_id'] ) : IPSMember::makeNameFormatted( $voter['members_display_name'], $voter['member_group_id'] ) ),
										 '_last'                => 0 );
		...
		if ( $this->request['view'] )
		{
			...
			else if ($this->request['view'] == 'findpost')
			{
				...
				if ( $pid > 0 )
				{
					...
					$this->DB->build( array( 'select' => 'COUNT(*) as posts', 'from' => 'posts', 'where' => "topic_id={$topicData['tid']} AND {$sort_field} <=" . intval( $sort_value ) . ' AND ' . $query ) );										
					$this->DB->execute();
					...
					{
						$search_hl .= "&amp;hl=" . urlencode( $this->request['hl'] );
					}
	...
	public function _getPosts()
	{
		...
		$_post_joins = array(
			...
			array(
				'select' => 'm.member_id as mid,m.name,m.member_group_id,m.email,m.joined,m.posts, m.last_visit, m.last_activity,m.login_anonymous,m.title as member_title, m.warn_level, m.warn_lastwarn, m.members_display_name, m.members_seo_name, m.member_banned, m.has_gallery, m.has_blog, m.members_bitoptions,m.mgroup_others'.$_extraMember,
				'from'   => array( 'members' => 'm' ),
		...
		$query = str_replace( '****FROM****', $this->DB->fetchSqlString(), $query );
		...
		while ( $row = $this->DB->fetch( $oq ) )
		{
			...
			(
				...
				or
				(
					isset( $this->memberData['forumsModeratorData'][ $forumData['id'] ] )
					and
					(
						( $row['approved'] == 0 and $this->memberData['forumsModeratorData'][ $forumData['id'] ]['post_q'] )
						or
						( $row['queued'] == 2 and $this->memberData['forumsModeratorData'][ $forumData['id'] ]['bw_mod_un_soft_delete'] )
						or
						( $row['queued'] == 0 and $this->memberData['forumsModeratorData'][ $forumData['id'] ]['bw_mod_soft_delete'] )
					)
				)
			)
			{
	...
	public function topicSetUp( $topicData )
	{
		...
		$topicData['SHOW_PAGES']
			= $this->registry->output->generatePagination( array(   'totalItems'		=> ($topicData['posts']+1),
																	'itemsPerPage'		=> $this->settings['display_max_posts'],
																	'currentPage'	    => intval( $this->request['page'] ),
