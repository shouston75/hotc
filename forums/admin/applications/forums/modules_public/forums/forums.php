...
class public_forums_forums_forums extends ipsCommand
{
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		if( $this->forum['show_rules'] == 2)
		{
			...
			$parser->set( array( 'memberData'     => array( 'member_group_id' => $this->settings['admin_group'] ),
							 	 'parseBBCode'    => true,
							 	 'parseArea'	  => 'rules',
							 	 'parseHtml'      => true,
							 	 'parseEmoticons' => true ) );
							 	 
			$this->forum['rules_text'] = $parser->display( $this->forum['rules_text'] );
	...
	public function renderForum()
	{
		...
		$other_data = array( 'forum_data'		=> $this->forum,
							 'hasMore'			=> ( $this->registry->class_forums->pageToSt( $this->request['page'] ) + $this->settings['display_max_topics'] > $total_possible['max'] ) ? false : true,
		...
		while( $t = $this->DB->fetch() )
		{
			...
			(
				...
				or
				(
					isset( $this->memberData['forumsModeratorData'][ $this->forum['id'] ] )
					and
					(
						( $t['pinned'] == 0 and $this->memberData['forumsModeratorData'][ $this->forum['id'] ]['pin_topic'] )
						or
						( $t['pinned'] == 1 and $this->memberData['forumsModeratorData'][ $this->forum['id'] ]['unpin_topic'] )
						or
						( $t['state'] == 'open' and $this->memberData['forumsModeratorData'][ $this->forum['id'] ]['close_topic'] )
						or
						( $t['state'] == 'closed' and $this->memberData['forumsModeratorData'][ $this->forum['id'] ]['open_topic'] )
					)
				)
			)
			{
