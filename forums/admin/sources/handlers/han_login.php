...
class han_login
{
	...
	public function verifyLogin()
	{
		...
		if ( $this->return_code != 'SUCCESS' )
		{
			...
			else if( $this->return_code == 'VALIDATING' )
			{
				return array( null, null, 'validating_remote' );
			}
			else
			{
	 ...
	 */
  	public function loginPasswordCheck( $username, $email_address, $password )
  	{
		foreach( $this->modules as $k => $obj_reference )
		{
			$obj_reference->is_password_check = TRUE;
	 ...
	 */
  	public function logoutCallback( $memberData )
  	{
		...
		foreach( $this->modules as $k => $obj_reference )
		{
			...
			{
				$returns[] = $obj_reference->logoutCallback( $memberData );
			}
	 ...
	 */
	public function changeName( $old_name, $new_name, $email_address, $memberData, $type='name' )
  	{
  		$this->return_code = 'METHOD_NOT_DEFINED';
		...
		foreach( $this->modules as $k => $obj_reference )
		{
			...
			{
				$obj_reference->changeName( $old_name, $new_name, $email_address, $memberData, $type );
				$this->return_code 		= $obj_reference->return_code;
			}
  		}
