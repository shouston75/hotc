 ...
 * IP.Board v3.4.1
 * API: Core
 * Last Updated: $Date: 2012-12-12 12:01:19 -0500 (Wed, 12 Dec 2012) $
 * </pre>
 *
...
class hanEmail
{
	...
	public function setPlainTextTemplate( $string, $unsub=false )
	{
		...
		/* Bug #39309 */
		( $unsub ) ? $this->plainTextTemplate = str_replace( '&nbsp;', ' ', $string ) . "\n\n\n\n<#UNSUBSCRIBE#>" : $this->plainTextTemplate = str_replace( '&nbsp;', ' ', $string );
	 ...
	 * @return	@e void
	 */
	public function buildHtmlContent( $words=array() )
	{
		/* Init */
		$htmlWords = array();
		...
		$this->htmlTemplate      = $this->applyHtmlWrapper( $this->subject, $this->convertTextEmailToHtmlEmail( $this->htmlTemplate ) );
		$this->htmlTemplate      = preg_replace( '#<!--hook\.([^\>]+?)-->#', '', $this->htmlTemplate );
		$this->htmlTemplate		 = $this->registry->getClass('output')->parseIPSTags( $this->htmlTemplate );
