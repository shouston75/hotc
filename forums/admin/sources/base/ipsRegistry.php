...
class ipsRegistry
{
	...
	static public function init()
	{
		...
		if ( IPS_AREA == 'admin' )
		{
			...
			if ( ( ipsRegistry::$request['module'] != 'login' ) AND ( ! $validationStatus ) )
			{
				...
				elseif ( ipsRegistry::$settings['logins_over_https'] && ( !$_SERVER['HTTPS'] and $_SERVER['HTTPS'] != 'off' ) )
				{
					/* Bug 38301 */
		...
		{	
			...
			/* Proper no cache key */
			ipsRegistry::$settings['noCacheKey'] = md5('$Rev: 11699 $');
	...
	static public function _loadAppCoreVariables( $appDir )
	{
		...
		if ( ! isset( self::$_coreVariables[ $appDir ] ) )
		{
			...
			if( is_file( $file ) )
			{
				...
				if ( IPS_AREA == 'admin' )
				{
					self::$_coreVariables[ $appDir ]['redirect'] = ( isset($_RESETACP) AND is_array( $_RESETACP ) ) ? $_RESETACP : array();
				}
				else
				{
	...
	protected static function _setUpAppData()
	{
		...
		$_appcomponent = preg_replace( '/[^a-zA-Z0-9\-\_]/', "" , ( isset( $_REQUEST['app'] ) && trim( (string) $_REQUEST['app'] ) ? $_REQUEST['app'] : IPS_DEFAULT_APP ) );
	...
	protected function setUpSettings()
	{
		...
		/* Save us a lot of bother later */
		define( 'IPS_IS_UTF8', ( IPS_DOC_CHAR_SET == 'UTF-8' ) ? true : false );
...
class ips_DBRegistry
{
	 ...
	 * @return string
	 */
	static public function getDatabaseName( $key='' )
	{
		/* Set Key */
		$key = ( $key ) ? $key : self::$defaultKey;
		
		return self::$dbObjects[ $key ]->obj['sql_database'];
	}
	...
	static public function setDB( $db_driver, $key='', $settings=array() )
	{
		...
		if ( ! class_exists( 'db_driver_' . $db_driver ) )
		{
			...
			{
				require_once( IPS_ROOT_PATH . 'sources/classes/backup/db.php' );
			
				$classname = 'ipsBackup_db';
			}
		}
...
/**
 * Class used to track visits from search enginesw
 */
class seoTracker
{
	/**
	...
	protected static $engines  = array(
		...
		array( 'name' => 'Bing', 'match' => '/http\:\/\/www\.bing/', 'parser' => 'parseBingUrl' ),
		array( 'name' => 'Bing', 'match' => '/http\:\/\/www\.bing/', 'parser' => 'parseBingUrl' ),
		array( 'name' => 'Facebook', 'match' => '/http\:\/\/www\.facebook/', 'parser' => 'parseFacebookUrl' )
	);
