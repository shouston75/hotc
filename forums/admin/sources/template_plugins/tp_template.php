 ...
 * Last Updated: $Date: 2012-09-27 06:09:08 -0400 (Thu, 27 Sep 2012) $
 * </pre>
 *
 * @author 		$Author: mark $
...
class tp_template extends output implements interfaceTemplatePlugins
{
	...
	public function runPlugin( $data, $options )
	{
		...
		$_group		= str_replace( '{search_app}', "'.\$this->request['search_app'].'", $_group );
		
		$_params	= $options['params'];

		$return		= "( method_exists( \$this->registry->getClass('output')->getTemplate('" . $_group . "'), '{$data}' ) ? \$this->registry->getClass('output')->getTemplate('" . $_group . "')->".$data."(".$_params.") : '' )";
