...
class class_text_parser_bbcode extends classes_text_parser
{
	...
	protected function _finishUrlsForDisplay( $txt )
	{
		...
		if ( preg_match( '#(http|https)://#', $txt ) && ! stristr( $txt, '<a' ) )
		{
			$txt = $this->_autoLinkUrls( $txt );
		}
		
		preg_match_all( '#<a href=["\']([^"\']+?)["\']([^>]+?)?>(.+?)</a>#is', $txt, $urlMatches );
		...
		for( $i = 0 ; $i < count( $urlMatches[0] ) ; $i++ )
		{
			...
			if ( $done !== true )
			{
				...
				/*
				 * Changes here @link
				 * http://community.invisionpower.com/tracker/issue-36082-long-links-on-mobile-extend-width/
				 */
				if ( ( empty( $this->settings['__noTruncateUrl'] ) ) and IPSText::mbstrlen( $text ) > 38 and ( substr( $text, 0, 7 ) == 'http://' or substr( $text, 0, 8 ) == 'https://' ) )
				{
