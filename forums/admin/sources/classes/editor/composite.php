...
class classes_editor_composite
{
	...
	*/	
	...
	public function process( $content )
	{
		...
		if ( $parser->testForParsingLimits( $content ) !== true )
		{
	...
	protected function _prePurify( $content )
	{
		...
		/* Take a stab at converting basic image tag */
	...
	private function _newPurifierObject()
	{
		...
		$noTags          = array( 'html', 'body', 'head', 'meta', 'title', 'script', 'style', 'frame', 'frameset', 'base', 'basefont', 'applet' );
				...
				/* P, DIV, PRE, BLOCQUOTE */
		$tags = 'p[style],div[style],pre[class],blockquote[class|data-author|data-cid|data-date|data-time|data-collapsed],'
				/* Span */
			  . 'span[style],br,'
			  	/* B, I, U, S, s, s */
		...
		if ( $this->getAllowHtml() && $this->getIsHtml() )
		{
			...
			$config->set('URI.SafeIframeRegexp', '%^http://((www.)?youtube.com/embed/|player.vimeo.com/video/)%'); //allow YouTube and Vimeo
