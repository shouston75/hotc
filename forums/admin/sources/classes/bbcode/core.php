...
class class_bbcode_core
{
	...
	public function preDbParse( $txt="" )
	{
		...
		$legacy = new class_text_parser_legacy();
		$txt = $legacy->preDbParse( $txt );
	...
	{	
		...
		$parser->set( array( 'memberData'     => array( 'member_group_id' => $this->parsing_mgroup, 'mgroup_others' => $this->parsing_mgroup_others ),
							 'parseBBCode'    => $this->parse_bbcode,
							 'parseArea'	  => $this->parsing_section,
							 'parseHtml'      => $this->parse_html,
							 'parseEmoticons' => $this->parse_smilies ) );
			...
			foreach( $this->_bbcodes[ $cur_method ] as $_bbcode )
			{
				...
				if( $_bbcode['bbcode_php_plugin'] )
				{
					...
					if( isset($this->plugins[ $_key ]) )
					{
						...
						{
							$_original	= $txt;
							$txt		= $this->plugins[ $_key ]->$method( $txt, 'bbcode' );
	...
	protected function _checkForEmbeddedCode( $matches )
	{
		...
		$parser = new classes_text_parser();
		$data   = $parser->getTagPositions( $txt, 'code' );
