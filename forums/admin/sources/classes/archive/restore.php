...
class classes_archive_restore
{
	...
	public function restore( $pids=array() )
	{
		...
		foreach( $topics as $tid => $data )
		{
			...
			else if ( $maxes[ $tid ] > $data['_maxPid'] )
			{
				/* If this is true, the local post table has a higher PID than the archive table
				   and this should never happen, so lets investigate */
				$_topic = $this->DB->buildAndFetch( array( 'select' => 'posts', 'from' => 'topics', 'where' => 'tid=' . intval( $tid ) ) );
				$_post  = $this->DB->buildAndFetch( array( 'select' => 'COUNT(*) as count', 'from' => 'posts', 'where' => 'topic_id=' . intval( $tid ) ) );
				
				/* Local database has all the data.. so lets call it done */
				if ( ( $_topic['posts'] + 1 ) == $_post['count'] )
				{
					$completedTopics[] = $data['tid'];
				}
				else
