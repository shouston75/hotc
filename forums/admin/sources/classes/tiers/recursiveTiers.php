...
class recursiveTiers
{
	...
	/**
	 * Custom method to build the tiers (can prevent SQL query)
	 *
	 * @access	protected
	 * @var		string
	 */
	protected $_customBuildMethod	= '';
	...
	protected function _buildSkinTiers()
	{
		...
		{
			$_cache	= $this->caches['skinsets'];
			
			if( !count($_cache) OR !is_array($_cache) )
			{
				$this->_customBuildMethod	= '';
				
				return $this->_buildTiers();
			}
			
			foreach( $_cache as $item )
			{
