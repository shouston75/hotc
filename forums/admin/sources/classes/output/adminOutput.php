 ...
 * Last Updated: $LastChangedDate: 2012-11-28 09:38:19 -0500 (Wed, 28 Nov 2012) $
 * </pre>
 *
...
class adminOutput extends output
{
	 ...
	 * @var		array
	 */
	private $tabKeys	  = array();
	private $otherTabKeys = array();
	private $tabPrefs     = array();
	
	/**
	 * Is bookmarkable
	 * @var	boolean
	 */
	protected $_canBookmark = true;
	...
	/**
	 * Called in admin function constructor to prevent output init clashes
	 */
	public function initTabKeys()
	{
		/* Get tab prefs first */
		$this->tabPrefs = $this->registry->adminFunctions->staffGetCookie( 'tabPrefs' );
		
		/* Set up default tab keys */
		$this->getMainTabKeys();
	}
	
	/**
	 * Returns keys for use in the main tab bar
	 * @return array:
	 */
	public function getMainTabKeys()
	{
		if ( ! count( $this->tabKeys ) )
		{
			if ( isset( $this->tabPrefs['tabBar'] ) && is_array( $this->tabPrefs['tabBar'] ) && count( $this->tabPrefs['tabBar'] ) )
			{
				$this->tabKeys = array_values( $this->tabPrefs['tabBar'] );
			}
			else
			{
				$this->tabKeys = array( 'core', 'forums', 'members', 'lookfeel', 'support', 'reports', 'other' );
			}
		}
	...
	/**
	 * Returns keys for use in the other apps menu
	 * @return array:
	 */
	public function getOtherTabKeys()
	{
		$tmp = $this->registry->getClass('class_permissions')->return;
		$this->registry->getClass('class_permissions')->return = TRUE; // We have to do this to stop the checkForAppAccess call below to end up sending a showError screen
				
		$mainTabKeys = $this->getMainTabKeys();
		
		if ( ! count( $this->otherTabKeys ) )
		{
			if ( isset( $this->tabPrefs['otherMenu'] ) && is_array( $this->tabPrefs['otherMenu'] ) && count( $this->tabPrefs['otherMenu'] ) )
			{
				$this->otherTabKeys = array_values( $this->tabPrefs['otherMenu'] );
				
				/* Got all apps? Some could have been added after we last re-arranged tabs */
				foreach( ipsRegistry::$applications as $app_dir => $app_data )
				{
					if ( $this->registry->getClass('class_permissions')->checkForAppAccess( $app_data['app_directory'] ) !== TRUE || ! $app_data['app_enabled'] )
					{
						continue;
					}
					
					if ( ! in_array( $app_dir, $mainTabKeys ) && ! in_array( $app_dir, $this->otherTabKeys ) )
					{
						$this->otherTabKeys[] = $app_data['app_directory'];
					}
				}
			}
			else
			{
				foreach( ipsRegistry::$applications as $app_dir => $app_data )
				{				
					if ( in_array( $app_data['app_directory'], array( 'core', 'forums', 'members' ) ) || in_array( $app_data['app_directory'], $mainTabKeys ) || $this->registry->getClass('class_permissions')->checkForAppAccess( $app_data['app_directory'] ) !== TRUE || ! $app_data['app_enabled'] )
					{
						continue;
					}
		...
		$this->registry->getClass('class_permissions')->return = $tmp;
		
		return $this->otherTabKeys;
	}
	
	/**
	 * Get tab data from keys
	 * @param	array	Array of keys to fetch data from
	 */
	public function getTabDataFromKeys( $keys )
	{
		$tmp = $this->registry->getClass('class_permissions')->return;
		$this->registry->getClass('class_permissions')->return = TRUE; // We have to do this to stop the checkForAppAccess call below to end up sending a showError screen
		...
		foreach( $keys as $tabkey )
		{
			...
			if ( $tabkey == 'other' )
			{
				...
				$tabs[ $tabkey ] = array( 'title'     => $this->lang->words['other_applications'],
										  'active'    => false,
										  'appDir'	  => false,
										  'isEnabled' => true,
										  'permCheck' => true,
										  'url'       => "{$this->settings['_base_url']}app=core&amp;module=applications&amp;section=applications&amp;do=applications_overview" );
			}
			...
			foreach( $fakeApps as $fa => $data )
			{
				if ( $fa == $tabkey )
				{
					...
					$tabs[ $tabkey ] = array( 'title'     => $title,
											  'permCheck' => $fakeAppAccess[ $fa ],
											  'active'    => $fakeAppActive[ $fa ],
											  'appDir'	  => $defaultFakeApp[ $fa ],
											  'isEnabled' => true,
											  'url'       => $link );
				}
			}
			
			/* Try normal apps next */
			if ( $gotIt === false )
			{
				/* Loop */
				foreach( $applications as $app_dir => $app_data )
				{
					switch( $app_data['app_directory'] )
					{
						case 'core':
							$app_data['app_title'] = $this->lang->words['gl_system'];
							break;
			
						case 'forums':
							$app_data['app_title'] = $this->lang->words['gl_forums'];
							break;
			
						case 'members':
							$app_data['app_title'] = $this->lang->words['gl_members'];
							break;
					}
						
					if ( $tabkey == $app_data['app_directory'] )
					{
						$tabs[ $tabkey ] = array( 'title'     => $app_data['app_title'],
												  'active'    => ( $fakeActive === false && ipsRegistry::$current_application == $app_data['app_directory'] ) ? true : false,
												  'permCheck' => ipsRegistry::getClass('class_permissions')->checkForAppAccess( $app_data['app_directory'] ),
												  'isEnabled' => (boolean) $app_data['app_enabled'],
												  'appDir'	  => $app_data['app_directory'],
												  'url'       => "{$this->settings['_base_url']}app={$app_data['app_directory']}" );
					}
				}
			}
		}
	...
	{
		$this->_canBookmark = (boolean) $bool;
	}
	 ...
	 */
	public function getXTemplate( $groupName )
	{
		if ( ! isset( $this->compiled_templates[ $groupName ] ) || ! is_object( $this->compiled_templates[ $groupName ] ) )
		{
			//-----------------------------------------
			...
			list( $app, $template ) = explode( '.', $groupName );
			
			self::loadTemplate( $template, $app );
		}
	
		return isset( $this->compiled_templates[ $groupName ] ) ? $this->compiled_templates[ $groupName ] : NULL;
	}
	...
	public function sendOutput( $return=false )
	{
		...
		$_bookmarkData = array( 'url' => $bookmarks->cleanUrl( $this->settings['query_string_safe'] ),
							    'can' => ( $this->_canBookmark ? 'true' : 'false' ),
								'has' => ( $bookmarks->hasBookmarked( $this->settings['query_string_safe'] ) ? 'true' : 'false' ),
								'jsn' => $bookmarks->asJson() );	
