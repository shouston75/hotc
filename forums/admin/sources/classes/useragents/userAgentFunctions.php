...
class userAgentFunctions
{
	...
	public function saveUserAgentFromEdit( $uagent_id, $uagent_key, $uagent_name, $uagent_regex, $uagent_regex_capture, $uagent_type, $uagent_position )
	{
		...
		$this->DB->update( 'core_uagents', array( 'uagent_key'           => $uagent_key,
												     ...
												     'uagent_regex'         => str_replace( '&#092;', '\\', $uagent_regex ),
	...
	public function findUserAgentID( $userAgent='' )
	{
		...
		else if ( stristr( $userAgent, 'IP-OTHER' ) ) // http://community.invisionpower.com/resources/bugs.html/_/ip-board/mobile-app-user-agents-invalid-r39508 
		{
			preg_match( '#^IP-OTHER\/([\d].[\d]+?)#i', $userAgent, $matches );
			if ( $matches[0] )
			{
				return array( 'uagent_id' => -1, 'uagent_key' => 'ipsMobileOther', 'uagent_name' => 'IPS Mobile Other', 'uagent_type' => 'mobileApp', 'uagent_version' => intval( $matches[1] ) );
			}
		}
