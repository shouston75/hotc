 ...
 * IP.Board v3.1.3
 * Global Search
 * Last Updated: $Date: 2010-09-27 18:18:35 -0400 (Mon, 27 Sep 2010) $
 * </pre>
 *
...
class searchPluginSphinxIndex implements iSearchIndexPlugin
{
	...
	private function _searchQuery( $search_term, $limit_clause, $sort_by, $group_by='', $count_only=false, $content_title_only=false )
	{
		...
		{
			/* Owner only */
			$this->sphinxClient->SetFilter( 'owner_only', array( 0, intval($this->memberData['member_id']) ) );
			...
			{
				$friends_ids[] = intval($r['friends_member_id']);
			}

			$this->sphinxClient->SetFilter( 'friend_only', $friends_ids );

			/* Authorized users only */
			$this->sphinxClient->SetFilter( 'authorized_users', array( 0, intval($this->memberData['member_id']) ) );
		}

		//-----------------------------------------
		// Allow app to modify query
		//-----------------------------------------
		
		if( ! isset( $this->display_plugins[ $this->request['search_app'] ] ) || ! $this->display_plugins[ $this->request['search_app'] ] )
		{
			if( file_exists( IPSLib::getAppDir( $this->request['search_app'] ) . '/extensions/searchDisplay.php' ) )
			{
				require_once( IPSLib::getAppDir( $this->request['search_app'] ) . '/extensions/searchDisplay.php' );
				$_class = strtolower($this->request['search_app']) . 'SearchDisplay';

				$this->display_plugins[ $this->request['search_app'] ] = new $_class();
			}
			else
			{
				return false;
		...
		$this->appSearchPlugin->onlyTitles		= $this->onlyTitles;
		$this->appSearchPlugin->noPostPreview	= $this->noPostPreview;

		$this->display_plugins[ $this->request['search_app'] ]->search_plugin	= $this->appSearchPlugin;

		if( method_exists( $this->display_plugins[ $this->request['search_app'] ], 'modifySearchQuery' ) )
		{
			/* Get the modified query */
			$this->display_plugins[ $this->request['search_app'] ]->modifySearchQuery( $this->sphinxClient, $count_only );
		}

		$groupby	= $this->onlyTitles ? true : false;
