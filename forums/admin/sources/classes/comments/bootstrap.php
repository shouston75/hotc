...
abstract class classes_comments_renderer
{
	...
	public function fetchFormatted( $parent, $filters=array() )
	{
		...
		{
			$preReply = '';
			foreach ( $this->request['_rcid'] as $_rcid )
			{
				$preReply .= $this->fetchReply( $parent['parent_id'], $_rcid, $this->memberData );
			}
		}
	...
	public function fetch( $parent, $filters=array() )
	{
		...
		while( $row = $this->DB->fetch( $oq ) )
		{
			...
			/* Set up parser */
			$this->parser->set( array( 'memberData' => $row ) );
			...
			/* Parse for display */
			$row['comment_text'] = $this->parser->display( $row['comment_text'] );
			...
			/* View */
			$row['_canView'] = ( $this->can('view', $row) === true ) ? 1 : 0;
