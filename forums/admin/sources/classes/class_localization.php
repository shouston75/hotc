 ...
 * Last Updated: $Date: 2012-12-10 11:40:55 -0500 (Mon, 10 Dec 2012) $
 * </pre>
 * 
    ...
    */	
    ...
    public function getDate($date, $method, $norelative=0, $full_relative=0, $overrideOffset=NULL)
    {
        ...
        if ( $full_relative and ( $norelative != 1 ) )
		{
			...
			{
				return sprintf( $this->words['time_weeks_ago'], intval($diff / 604800) );
			}
			else
			{
	 ...
	 */
	 ...
	 {
	 	parse_str( str_replace( '&amp;', '&', $this->settings['query_string_formatted'] ), $params );
	 	
	 	unset( $params['k'] );
	 	unset( $params['setlanguage'] );
	 	unset( $params['langurlbits'] );
	 	unset( $params['cal_id'] );
	 	unset( $params['langid'] );
	 
		return ipsRegistry::getClass('output')->buildUrl( str_replace( '&', '&amp;', http_build_query( $params ) ) . "&amp;k={$this->member->form_hash}&amp;setlanguage=1&amp;langurlbits={$this->settings['query_string_safe']}&amp;cal_id={$this->request['cal_id']}&amp;langid={$langId}", 'public' );
	 }
