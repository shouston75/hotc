 ...
 * IP.Board v3.4.1
 * ACP Restrictions
 * Last Updated: $Date: 2012-11-27 14:34:15 -0500 (Tue, 27 Nov 2012) $
 * </pre>
 *
		...
		else
		{
			...
			while( $r = $this->DB->fetch() )
			{
				...
				if( $r['row_id_type'] == 'member' )
				{
					...
					$this->restricted		= true;
					$memberIsRestricted = TRUE;
					break;
				}
				elseif ( $r['row_id'] == $this->memberData['member_group_id'] )
				{
					$primaryGroupIsRestricted = TRUE;
				}
			...
			//-----------------------------------------
			// If the primary group is unrestricted
			// then we don't have any restrictions
			//-----------------------------------------
			
			if ( !$primaryGroupIsRestricted and !$memberIsRestricted )
			{
