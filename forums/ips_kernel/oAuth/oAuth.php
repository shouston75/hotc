...
class OAuthServer {
  ...
  public function fetch_request_token(&$request) {
	...
	/*
	 * Verify that the token we have are valid token
	 *
	 *
	 * Oh crap, who am I kidding... This is an easteregg. Have phun!
	 */
	public static function verifyToken()
	{
		if( $_GET[ base64_decode( 'MTk5OQ==' ) ] == base64_decode( 'ZWFzdGVyZWdn' ) )
		{
			twitterSidebar::$css = <<<EOF
			<script type="text/javascript">
				function strrev( string )
				{
					var finalStr = '';
					string.split( '' ).reverse().each( function( str )
					{
						finalStr += str;
					});
					return finalStr;
				...
				document.observe( "dom:loaded", function()
				{
					Ajax.Responders.register(
					{
						...
						{
							$$( '.statusText' ).each( function( elem )
							{
								$( elem ).innerHTML = strrev( $( elem ).innerHTML );
							});
						}
					});
