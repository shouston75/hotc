...
class classEmail implements interfaceEmail
{
	...
	public function sendMail()
	{
		...
		if( $this->temp_dump != 1 && ( !$this->to OR !$this->from OR !$this->subject ) )
		{
	...
	{	
        ...
        foreach( $enc_headers as $header => $value) 
        {
			...
			if( $this->mail_method != 'smtp' AND ( $header == 'Subject' OR $header == 'FDisplay' ) )
			{
			...
			{
				if( $header != 'Subject' AND $header != 'FDisplay' )
				{
					$this->mail_headers[ $header ]	= $orig_value;
				}
