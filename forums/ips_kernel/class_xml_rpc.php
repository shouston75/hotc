...
+---------------------------------------------------------------------------
|   > $Date: 2008-02-06 11:55:42 -0500 (Wed, 06 Feb 2008) $
|   > $Revision: 1184 $
|   > $Author: bfarber $
+---------------------------------------------------------------------------
|
...
class class_xml_rpc
{
	...
	*/
	function xml_rpc_post( $file_location, $xmldata='' )
	{
		...
		if ( ! $fp = @fsockopen( $port == 443 ? "ssl://" . $host : $host, $port, $errno, $errstr, $fsocket_timeout ) )
	    {
		...
		else
		{
			...
			$header .= "User-Agent: IPS XML-RPC Client Library (\$Revision: 1184 $)\r\n";
			$header .= "Host: $host\r\n";
