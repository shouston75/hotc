	...
	function mod_find_user_complete()
	{
 	...
 	function attachments()
 	{
		$info     = array();
 		...
 		if ( $maxspace > 0 )
 		{
			...
			$this->ipsclass->lang['attach_space_count'] = sprintf( $this->ipsclass->lang['attach_space_count'], intval($stats['count']), intval($info['full_percent']) );
 	...
 	function email_change($msg="")
 	{
		...
		$msg = $msg ? $this->ipsclass->lang[$msg] : '';
 		...
 		if ($this->ipsclass->vars['bot_antispam'])
 		{
 			$this->output = str_replace( "<!--ANTIBOT-->", $captchaHTML, $this->output );
 		}
 	...
 	function avatar()
 	{
 	if( $this->ipsclass->vars['buy_ava'] == 1 AND $this->ipsclass->member['pava'] == 0 )
		{
			$this->ipsclass->Error( array( 'MSG' => 'no_ava_perm' ) );
		}
		//-----------------------------------------
		// Check to make sure that we can edit profiles..
		//-----------------------------------------
 	...
 	function signature()
 	{
 	if( $this->ipsclass->vars['buy_sig'] == 1 AND $this->ipsclass->member['psig'] == 0 )
		{
			$this->ipsclass->Error( array( 'MSG' => 'no_sig_perm' ) );
		}
		//-----------------------------------------
		// Check to make sure that we can edit profiles..
		//-----------------------------------------
