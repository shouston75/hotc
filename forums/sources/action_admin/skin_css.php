...
class ad_skin_css
{
	...
	*/
	...
	function colouredit()
	{
		...
		preg_match_all( "/@import\s+(.+?)$/sm", $css, $imports );
		
		$i = 5;
		
		if( is_array( $imports[1] ) AND count($imports[1]) )
		{
		...
		//print "<pre>"; print_r($imports); exit();
		//print "<pre>"; print_r($hiddens); exit();
		//-----------------------------------------
		// Make http:// safe..
		//-----------------------------------------
		...
		if( count($hiddens) )
		{
			$formfields = $formfields + $hiddens;
		}
		
		$this->ipsclass->html .= $this->ipsclass->adskin->start_form( $formfields );
