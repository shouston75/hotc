...
#print $chrs.' '.var_dump($replace).var_dump( $t );exit;
	...
	*/
	function regex_build_url( $url=array() )
	{
		...
		if ( strpos($url['html'],'www.homeofthechiefs.com' ) ) {
        return ( isset($url['st']) ? $url['st'] : '' ) . "<a href=\"".$url['html']."\" target=\"_self\">".$show."</a>" . $url['end'];
        }
        else {
	...
	*/
	...
	function clean_video_tag( $matches=array() )
	{
		...
		if (strstr($code, 'google') != "")
		{
			...
			return '[video]http://video.google.com/videoplay?docid='.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'youtube') != "")
		{
			$startingpos = strpos($code, '/v/') + 3;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://www.youtube.com/watch?v='.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'liveleak') != "")
		{
			$startingpos = strpos($code, 'token=') + 6;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://www.liveleak.com/view?i='.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'veoh') != "")
		{
			$startingpos = strpos($code, 'permalinkId=') + 12;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://www.veoh.com/videos/'.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'gametrailers') != "")
		{
			$startingpos = strpos($code, 'mid=') + 4;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://www.gametrailers.com/player/'.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'ifilm') != "")
		{
			$startingpos = strpos($code, 'flvbaseclip=') + 12;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://www.ifilm.com/video/'.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'revver') != "")
		{
			$startingpos = strpos($code, 'mediaId=') + 8;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://one.revver.com/watch/'.substr($code, $startingpos, $lenght).'[/video]';
		}
                else if (strstr($code, 'brightcove') != "")
		{
			$startingpos = strpos($code, 'initVideoId=') + 12;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://www.brightcove.com/title.jsp?title='.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'myspace') != "")
		{
			$startingpos = strpos($code, 'm=') + 2;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://vids.myspace.com/index.cfm?fuseaction=vids.individual&videoid='.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'collegehumor') != "")
		{
			$startingpos = strpos($code, 'clip_id=') + 8;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://www.collegehumor.com/video:'.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'vsocial') != "")
		{
			$startingpos = strpos($code, '?d=') + 3;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://www.vsocial.com/video/?d='.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'guba') != "")
		{
			$startingpos = strpos($code, '/uploaditem/') + 12;
			$lenght = strpos($code, '/flash.flv') - $startingpos;
			return '[video]http://www.guba.com/watch/'.substr($code, $startingpos, $lenght).'[/video]';
		}
		else if (strstr($code, 'glumbert') != "")
		{
			$startingpos = strpos($code, '/embed/') + 7;
			$lenght = strpos($code, '&') - $startingpos;
			return '[video]http://www.glumbert.com/media/'.substr($code, $startingpos, $lenght).'[/video]';
		}
	}
