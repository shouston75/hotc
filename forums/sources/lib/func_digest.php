...
class func_digest
{
	...
	function func_digest_init( $ROOT_PATH )
	{
		...
		$this->ipsclass->forums->strip_invisible = 0;
	...
	function _send_topic_digest()
	{
		...
		while( $t = $this->ipsclass->DB->fetch_row( $topic_query ) )
		{
			...
			if( $t['mgroup_others'] )
			{
				$t['mgroup_others'] = $this->ipsclass->clean_perm_string( $t['mgroup_others'] );
			...
			$perm_id = ( $t['org_perm_id'] ) ? $t['org_perm_id'] : $this->ipsclass->cache['group_cache'][ $t['mgroup'] ]['g_perm_id'].$mgroup_others;
			
			if ( $this->ipsclass->forums->forum_by_id[ $t['forum_id'] ]['read_perms'] != '*' )
			{
				if ( ! preg_match("/(^|,)".str_replace( ",", '|', $perm_id )."(,|$)/", $this->ipsclass->forums->forum_by_id[ $t['forum_id'] ]['read_perms'] ) )
    			{
    				continue;
   				}
			}
			...
			if( $t['approved'] == 0 )
			{
				...
				if( $this->ipsclass->cache['group_cache'][$t['mgroup']]['g_is_supmod'] == 1 )
				{
					$mod = 1;
				}
				else if ( count($this->ipsclass->cache['moderators']) )
				{
					...
					if( $t['mgroup_others'] )
					{
						$other_mgroups = explode( ",", $this->ipsclass->clean_perm_string( $t['mgroup_others'] ) );
					}
					
					foreach( $this->ipsclass->cache['moderators'] as $moderators )
					{
						if ( $moderators['member_id'] == $t['id'] or $moderators['group_id'] == $t['mgroup'] )
						{
							$mod = 1;
						}
			...
			{
				$topic_title = $t['title'];
				$forum_name  = $this->ipsclass->forums->forum_by_id[ $t['forum_id'] ]['name'];
