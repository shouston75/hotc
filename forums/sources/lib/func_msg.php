 	...
 	function _get_real_allowance( $member )
 	{
 		...
 		if ( count( $groups_id ) )
		{
			foreach( $groups_id as $pid )
			{
				...
				if ( $member['g_max_messages'] > 0 AND $this->ipsclass->cache['group_cache'][ $pid ]['g_max_messages'] > $member['g_max_messages'] )
				{
					$member['g_max_messages'] = $this->ipsclass->cache['group_cache'][ $pid ]['g_max_messages'];
				}
				else if ( $this->ipsclass->cache['group_cache'][ $pid ]['g_max_messages'] == 0 )
				{
