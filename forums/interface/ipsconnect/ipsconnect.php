...
class ipsConnect
{
	...
	public function login( $identifier, $identifierValue, $md5Password, $key, $redirect, $redirectHash )
	{
		...
		if ( in_array( $identifier, array( 'id', 'email', 'username' ) ) )
		{
			...
			if ( $member['member_id'] )
			{
				...
				{		
					...
					{
						/* Are we validating? */
						$validating = ipsRegistry::DB()->buildAndFetch( array( 'select' => '*', 'from' => 'validating', 'where' => "member_id={$member['member_id']} AND lost_pass=0" ) );
						if ( $validating['vid'] )
						{
		...
		$this->_return( $redirect, array( 'connect_status' => $statusCode, 'connect_id' => $member['member_id'], 'connect_username' => $member['name'], 'connect_displayname' => $member['members_display_name'], 'connect_email' => $member['email'], 'connect_unlock' => $secondsUntilUnlock ) );
	}
	 ...
	 */
	public function register( $key, $username, $displayname, $md5Password, $email )
	{
	...
	public function cookies( $data )
	{
		...
		if ( isset( $cookies[ ipsRegistry::$settings['cookie_id'] . 'member_id' ] ) )
		{
			...
			if ( $member['member_id'] and $member['member_login_key'] == $cookies[ ipsRegistry::$settings['cookie_id'] . 'pass_hash' ] and time() <= $member['member_login_key_expire'] )
			{
				$statusCode = 'SUCCESS';
...
$map = array(
	...
	'register'	=> array( 'key', 'username', 'displayname', 'password', 'email' ),
	'cookies'	=> array( 'data' ),
